...
int main() {
    ...
    int m=4, n=4;
    double hA[16];
    for(int i=0;i<16;i++) hA[i]=(double)(i%4+1);
    ...
    CHECK_CUBLAS(UPTKblasDgemv(handle,UPTKBLAS_OP_N,m,n,&ha,dA,m,dx,1,&hb,dy,1));
    CHECK_CUBLAS(UPTKblasDgemv(handle,UPTKBLAS_OP_T,m,n,&ha,dA,m,dx,1,&hb,dy,1));
    UPTKFree(dA);UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDgemv_v2 PASS\n");
    return 0;
}
