...
int main() {
    ...
    int m=4,n=4;
    float hA[16],hX[4],hC[16];
    for(int i=0;i<16;i++) hA[i]=(float)(i+1);
    for(int i=0;i<4;i++) hX[i]=(float)(i+1);
    for(int i=0;i<16;i++) hC[i]=0;
    float *dA=NULL,*dX=NULL,*dC=NULL;
    UPTKMalloc(&dA,16*sizeof(float));UPTKMalloc(&dX,4*sizeof(float));UPTKMalloc(&dC,16*sizeof(float));
    ...
    CHECK_CUBLAS(UPTKblasSdgmm(handle,UPTKBLAS_SIDE_LEFT,m,n,dA,m,dX,1,dC,m));
    CHECK_CUBLAS(UPTKblasSdgmm(handle,UPTKBLAS_SIDE_RIGHT,m,n,dA,m,dX,1,dC,m));
    UPTKFree(dA);UPTKFree(dX);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSdgmm PASS\n");
    return 0;
}
