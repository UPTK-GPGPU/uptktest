...
int main() {
    ...
    cuComplex *dAP=NULL,*dx=NULL,*dy=NULL;
    UPTKMalloc(&dAP,10*sizeof(cuComplex));UPTKMalloc(&dx,4*sizeof(cuComplex));UPTKMalloc(&dy,4*sizeof(cuComplex));
    ...
    CHECK_CUBLAS(UPTKblasChpmv(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dAP,dx,1,&hb,dy,1));
    UPTKFree(dAP);UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasChpmv_v2 PASS\n");
    return 0;
}
