...
int main() {
    ...
    cuComplex *dA=NULL,*dAP=NULL;
    UPTKMalloc(&dA,16*sizeof(cuComplex));UPTKMalloc(&dAP,10*sizeof(cuComplex));
    UPTKMemcpy(dA,hA,16*sizeof(cuComplex),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasCtrttp(handle,UPTKBLAS_FILL_MODE_LOWER,n,dA,n,dAP));
    UPTKFree(dA);UPTKFree(dAP);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCtrttp PASS\n");
    return 0;
}
