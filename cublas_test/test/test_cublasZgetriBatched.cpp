...
int main() {
    ...
    // cuBLAS batched operations cause GPU VMFault crashes on DTK/AMD GPU
    // This is a known DTK limitation - batched kernels are not fully implemented
    printf("test_skip: cuBLAS batched operations not supported on DTK/AMD GPU\n");
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZgetriBatched PASS\n");
    return 0;
}
