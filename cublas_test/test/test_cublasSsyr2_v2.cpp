...
int main() {
    ...
    float ha=1.0f;
    float *dx=NULL,*dy=NULL,*dA=NULL;
    UPTKMalloc(&dx,4*sizeof(float));UPTKMalloc(&dy,4*sizeof(float));UPTKMalloc(&dA,16*sizeof(float));
    ...
    CHECK_CUBLAS(UPTKblasSsyr2(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dy,1,dA,n));
    UPTKFree(dx);UPTKFree(dy);UPTKFree(dA);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSsyr2_v2 PASS\n");
    return 0;
}
