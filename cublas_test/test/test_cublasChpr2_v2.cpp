...
int main() {
    ...
    cuComplex *dx=NULL,*dy=NULL,*dAP=NULL;
    UPTKMalloc(&dx,4*sizeof(cuComplex));UPTKMalloc(&dy,4*sizeof(cuComplex));UPTKMalloc(&dAP,10*sizeof(cuComplex));
    ...
    CHECK_CUBLAS(UPTKblasChpr2(handle,UPTKBLAS_FILL_MODE_LOWER,n,&alpha,dx,1,dy,1,dAP));
    UPTKFree(dx);UPTKFree(dy);UPTKFree(dAP);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasChpr2_v2 PASS\n");
    return 0;
}
