...
int main() {
    ...
    int n=4;
    cuDoubleComplex hAP[10],hx[4],hy[4];
    ...
    cuDoubleComplex *dAP=NULL,*dx=NULL,*dy=NULL;
    UPTKMalloc(&dAP,10*sizeof(cuDoubleComplex));UPTKMalloc(&dx,4*sizeof(cuDoubleComplex));UPTKMalloc(&dy,4*sizeof(cuDoubleComplex));
    ...
    CHECK_CUBLAS(UPTKblasZhpmv(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dAP,dx,1,&hb,dy,1));
    UPTKFree(dAP);UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZhpmv_v2 PASS\n");
    return 0;
}
