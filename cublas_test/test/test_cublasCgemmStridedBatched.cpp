...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasCgemmStridedBatched(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,m,16,dB,m,16,&hb,dC,m,16,batchCount));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCgemmStridedBatched PASS\n");
    return 0;
}
