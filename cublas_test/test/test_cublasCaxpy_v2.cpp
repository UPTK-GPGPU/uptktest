...
int main() {
    ...
    int n=4;
    cuComplex ha={2,0},hx[4],hy[4];
    for(int i=0;i<n;i++){hx[i].x=(float)(i+1);hx[i].y=0;hy[i].x=(float)(i+5);hy[i].y=0;}
    ...
    CHECK_CUBLAS(UPTKblasCaxpy(handle,n,&ha,dx,1,dy,1));
    UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCaxpy_v2 PASS\n");
    return 0;
}
