...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasSetAtomicsMode(handle, UPTKBLAS_ATOMICS_NOT_ALLOWED));
    CHECK_CUBLAS(UPTKblasSetAtomicsMode(handle, UPTKBLAS_ATOMICS_ALLOWED));
    CHECK_CUBLAS(UPTKblasSetAtomicsMode(handle, UPTKBLAS_ATOMICS_NOT_ALLOWED));
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSetAtomicsMode PASS\n");
    return 0;
}
