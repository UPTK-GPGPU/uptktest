...
int main() {
    ...
    float hx[4]={1,2,3,4};
    float ha=1.0f;
    float *dx=NULL,*dA=NULL;
    UPTKMalloc(&dx,4*sizeof(float));UPTKMalloc(&dA,16*sizeof(float));
    UPTKMemcpy(dx,hx,4*sizeof(float),UPTKMemcpyHostToDevice);
    UPTKMemset(dA,0,16*sizeof(float));
    CHECK_CUBLAS(UPTKblasSsyr(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dA,n));
    CHECK_CUBLAS(UPTKblasSsyr(handle,UPTKBLAS_FILL_MODE_UPPER,n,&ha,dx,1,dA,n));
    UPTKFree(dx);UPTKFree(dA);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSsyr_v2 PASS\n");
    return 0;
}
