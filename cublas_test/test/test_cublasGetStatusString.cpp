...
int main() {
    ...
    const char* s = UPTKblasGetStatusString(UPTKBLAS_STATUS_SUCCESS);
    printf("  str: %s\n", s);
    s = UPTKblasGetStatusString(UPTKBLAS_STATUS_NOT_INITIALIZED);
    printf("  str: %s\n", s);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasGetStatusString PASS\n");
    return 0;
}
