...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasSgeam(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    CHECK_CUBLAS(UPTKblasSgeam(handle,UPTKBLAS_OP_T,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSgeam PASS\n");
    return 0;
}
