...
int main() {
    ...
    float ha=1.0f;
    cuComplex *dx=NULL,*dA=NULL;
    UPTKMalloc(&dx,4*sizeof(cuComplex));UPTKMalloc(&dA,16*sizeof(cuComplex));
    UPTKMemcpy(dx,hx,4*sizeof(cuComplex),UPTKMemcpyHostToDevice);
    UPTKMemset(dA,0,16*sizeof(cuComplex));
    CHECK_CUBLAS(UPTKblasCher(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dA,n));
    UPTKFree(dx);UPTKFree(dA);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCher_v2 PASS\n");
    return 0;
}
