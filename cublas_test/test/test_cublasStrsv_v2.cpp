...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasStrsv(handle,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,n,dA,n,dy,1));
    CHECK_CUBLAS(UPTKblasStrsv(handle,UPTKBLAS_FILL_MODE_UPPER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,n,dA,n,dy,1));
    UPTKFree(dA);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasStrsv_v2 PASS\n");
    return 0;
}
