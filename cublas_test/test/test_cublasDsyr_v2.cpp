...
int main() {
    ...
    UPTKMalloc(&dx,4*sizeof(double));UPTKMalloc(&dA,16*sizeof(double));
    UPTKMemcpy(dx,hx,4*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemset(dA,0,16*sizeof(double));
    CHECK_CUBLAS(UPTKblasDsyr(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dA,n));
    UPTKFree(dx);UPTKFree(dA);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDsyr_v2 PASS\n");
    return 0;
}
