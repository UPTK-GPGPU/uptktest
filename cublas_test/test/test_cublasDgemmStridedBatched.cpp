...
int main() {
    ...
    double hA[32],hB[32],hC[32];
    for(int i=0;i<32;i++){hA[i]=(double)(i%4+1);hB[i]=(double)(i%4+1);hC[i]=0;}
    double ha=1.0,hb=0.0;
    double *dA=NULL,*dB=NULL,*dC=NULL;
    UPTKMalloc(&dA,32*sizeof(double));UPTKMalloc(&dB,32*sizeof(double));UPTKMalloc(&dC,32*sizeof(double));
    UPTKMemcpy(dA,hA,32*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dB,hB,32*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dC,hC,32*sizeof(double),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasDgemmStridedBatched(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,m,16,dB,m,16,&hb,dC,m,16,batchCount));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDgemmStridedBatched PASS\n");
    return 0;
}
