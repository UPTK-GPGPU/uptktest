...
int main() {
    ...
    double *dAP=NULL,*dy=NULL;
    UPTKMalloc(&dAP,10*sizeof(double));UPTKMalloc(&dy,4*sizeof(double));
    UPTKMemcpy(dAP,hAP,10*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dy,hy,4*sizeof(double),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasDtpmv(handle,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,n,dAP,dy,1));
    UPTKFree(dAP);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDtpmv_v2 PASS\n");
    return 0;
}
