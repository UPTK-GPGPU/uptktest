...
int main() {
    ...
    int m=4,n=4;
    cuDoubleComplex hA[16],hX[4],hC[16];
    for(int i=0;i<16;i++){hA[i].x=(double)(i+1);hA[i].y=0;}
    for(int i=0;i<4;i++){hX[i].x=(double)(i+1);hX[i].y=0;}
    for(int i=0;i<16;i++){hC[i].x=0;hC[i].y=0;}
    cuDoubleComplex *dA=NULL,*dX=NULL,*dC=NULL;
    UPTKMalloc(&dA,16*sizeof(cuDoubleComplex));UPTKMalloc(&dX,4*sizeof(cuDoubleComplex));UPTKMalloc(&dC,16*sizeof(cuDoubleComplex));
    ...
    CHECK_CUBLAS(UPTKblasZdgmm(handle,UPTKBLAS_SIDE_LEFT,m,n,dA,m,dX,1,dC,m));
    CHECK_CUBLAS(UPTKblasZdgmm(handle,UPTKBLAS_SIDE_RIGHT,m,n,dA,m,dX,1,dC,m));
    UPTKFree(dA);UPTKFree(dX);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZdgmm PASS\n");
    return 0;
}
