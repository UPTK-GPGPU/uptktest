...
int main() {
    ...
    for(int i=0;i<16;i++){hA[i].x=(double)(i+1);hA[i].y=0;hC[i].x=0;hC[i].y=0;}
    cuDoubleComplex ha={1,0},hb={0,0};
    CHECK_CUBLAS(UPTKblasXtZsyrk(handle,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,n,k,&ha,hA,n,&hb,hC,n));
    CHECK_CUBLAS(UPTKblasXtDestroy(handle));
    printf("test_cublasXtZsyrk PASS\n");
    return 0;
}
