...
int main() {
    ...
    for(int i=0;i<16;i++){hA[i]=(float)(i+1);hB[i]=(float)(i+1);hC[i]=0;}
    float ha=1.0f,hb=0.0f;
    float *dA=NULL,*dB=NULL,*dC=NULL;
    UPTKMalloc(&dA,16*sizeof(float));UPTKMalloc(&dB,16*sizeof(float));UPTKMalloc(&dC,16*sizeof(float));
    ...
    CHECK_CUBLAS(UPTKblasSsymm(handle,UPTKBLAS_SIDE_LEFT,UPTKBLAS_FILL_MODE_LOWER,m,n,&ha,dA,m,dB,m,&hb,dC,m));
    CHECK_CUBLAS(UPTKblasSsymm(handle,UPTKBLAS_SIDE_RIGHT,UPTKBLAS_FILL_MODE_LOWER,m,n,&ha,dA,n,dB,m,&hb,dC,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSsymm_v2 PASS\n");
    return 0;
}
