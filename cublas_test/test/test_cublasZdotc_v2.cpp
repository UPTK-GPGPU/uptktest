...
int main() {
    ...
    for(int i=0;i<n;i++){hx[i].x=(double)(i+1);hx[i].y=(double)i;hy[i].x=(double)(i+5);hy[i].y=(double)(i+1);}
    ...
    cuDoubleComplex r;
    CHECK_CUBLAS(UPTKblasZdotc(handle,n,dx,1,dy,1,&r));
    printf("  zdotc: (%f,%f)\n",r.x,r.y);
    UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZdotc_v2 PASS\n");
    return 0;
}
