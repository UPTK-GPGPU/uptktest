...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasSgemm(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,m,dB,m,&hb,dC,m));
    CHECK_CUBLAS(UPTKblasSgemm(handle,UPTKBLAS_OP_T,UPTKBLAS_OP_N,m,n,k,&ha,dA,m,dB,m,&hb,dC,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSgemm_v2 PASS\n");
    return 0;
}
