...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasCtrmm(handle,UPTKBLAS_SIDE_LEFT,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,m,n,&ha,dA,m,dB,m,dC,m));
    UPTKFree(dA);UPTKFree(dB);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCtrmm_v2 PASS\n");
    return 0;
}
