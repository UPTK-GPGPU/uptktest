...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasGemmEx(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,UPTK_R_32F,m,dB,UPTK_R_32F,m,&hb,dC,UPTK_R_32F,m,UPTKBLAS_COMPUTE_32F,UPTKBLAS_GEMM_DEFAULT));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasGemmEx PASS\n");
    return 0;
}
