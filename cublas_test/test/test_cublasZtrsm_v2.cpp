...
int main() {
    ...
    int m=4,n=4;
    cuDoubleComplex hA[16],hB[16];
    for(int i=0;i<16;i++){hA[i].x=(float)(i+1);hA[i].y=0;hB[i].x=(float)(i+1);hB[i].y=0;}
    cuDoubleComplex ha={1,0};
    cuDoubleComplex *dA=NULL,*dB=NULL;
    UPTKMalloc(&dA,16*sizeof(cuDoubleComplex));UPTKMalloc(&dB,16*sizeof(cuDoubleComplex));
    ...
    CHECK_CUBLAS(UPTKblasZtrsm(handle,UPTKBLAS_SIDE_LEFT,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,m,n,&ha,dA,m,dB,m));
    UPTKFree(dA);UPTKFree(dB);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZtrsm_v2 PASS\n");
    return 0;
}
