...
int main() {
    ...
    UPTKMalloc(&dA,32*sizeof(cuComplex));UPTKMalloc(&dB,32*sizeof(cuComplex));UPTKMalloc(&dC,32*sizeof(cuComplex));
    UPTKMemcpy(dA,hA,32*sizeof(cuComplex),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dB,hB,32*sizeof(cuComplex),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dC,hC,32*sizeof(cuComplex),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasCgemm3mStridedBatched(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,m,16,dB,m,16,&hb,dC,m,16,batchCount));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCgemm3mStridedBatched PASS\n");
    return 0;
}
