...
int main() {
    ...
    double ha=1.0;
    cuDoubleComplex *dx=NULL,*dAP=NULL;
    UPTKMalloc(&dx,4*sizeof(cuDoubleComplex));UPTKMalloc(&dAP,10*sizeof(cuDoubleComplex));
    UPTKMemcpy(dx,hx,4*sizeof(cuDoubleComplex),UPTKMemcpyHostToDevice);
    UPTKMemset(dAP,0,10*sizeof(cuDoubleComplex));
    CHECK_CUBLAS(UPTKblasZhpr(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dAP));
    UPTKFree(dx);UPTKFree(dAP);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZhpr_v2 PASS\n");
    return 0;
}
