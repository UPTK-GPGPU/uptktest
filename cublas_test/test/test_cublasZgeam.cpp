...
int main() {
    ...
    cuDoubleComplex ha={1,0},hb={0.5,0};
    ...
    CHECK_CUBLAS(UPTKblasZgeam(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    CHECK_CUBLAS(UPTKblasZgeam(handle,UPTKBLAS_OP_T,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZgeam PASS\n");
    return 0;
}
