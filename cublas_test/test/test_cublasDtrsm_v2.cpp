...
int main() {
    ...
    int m=4,n=4;
    double hA[16],hB[16];
    for(int i=0;i<16;i++){hA[i]=(double)(i+1);hB[i]=(double)(i+1);}
    double ha=1.0;
    double *dA=NULL,*dB=NULL;
    UPTKMalloc(&dA,16*sizeof(double));UPTKMalloc(&dB,16*sizeof(double));
    UPTKMemcpy(dA,hA,16*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dB,hB,16*sizeof(double),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasDtrsm(handle,UPTKBLAS_SIDE_LEFT,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,UPTKBLAS_DIAG_NON_UNIT,m,n,&ha,dA,m,dB,m));
    UPTKFree(dA);UPTKFree(dB);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDtrsm_v2 PASS\n");
    return 0;
}
