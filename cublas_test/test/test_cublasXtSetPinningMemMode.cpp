...
int main() {
    ...
    status = UPTKblasXtSetPinningMemMode(handle, UPTKBLASXT_PINNING_ENABLED);
    if (status != UPTKBLAS_STATUS_SUCCESS) {
        printf("test_skip: cublasXtSetPinningMemMode returned %s (Xt not fully supported on DTK)\n",
               UPTKblasGetStatusString(status));
    ...
    UPTKblasXtDestroy(handle);
    printf("test_cublasXtSetPinningMemMode PASS\n");
    return 0;
}
