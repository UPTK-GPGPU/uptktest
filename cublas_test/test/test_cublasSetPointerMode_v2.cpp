...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasSetPointerMode(handle, UPTKBLAS_POINTER_MODE_HOST));
    CHECK_CUBLAS(UPTKblasSetPointerMode(handle, UPTKBLAS_POINTER_MODE_DEVICE));
    CHECK_CUBLAS(UPTKblasSetPointerMode(handle, UPTKBLAS_POINTER_MODE_HOST));
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSetPointerMode_v2 PASS\n");
    return 0;
}
