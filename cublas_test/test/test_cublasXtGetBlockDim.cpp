...
int main() {
    ...
    status = UPTKblasXtSetBlockDim(handle, 256);
    if (status != UPTKBLAS_STATUS_SUCCESS) {
        printf("test_skip: cublasXtSetBlockDim returned %s (Xt not fully supported on DTK)\n",
               UPTKblasGetStatusString(status));
    ...
    int blockDim = 0;
    status = UPTKblasXtGetBlockDim(handle, &blockDim);
    if (status != UPTKBLAS_STATUS_SUCCESS) {
        printf("test_skip: cublasXtGetBlockDim returned %s (Xt not fully supported on DTK)\n",
               UPTKblasGetStatusString(status));
        UPTKblasXtDestroy(handle);
        return 0;
    }
    ...
    UPTKblasXtDestroy(handle);
    printf("test_cublasXtGetBlockDim PASS\n");
    return 0;
}
