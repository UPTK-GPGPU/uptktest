...
int main() {
    ...
    status = UPTKblasXtSetCpuRatio(handle, UPTKBLASXT_GEMM, UPTKBLASXT_FLOAT, 0.5f);
    if (status != UPTKBLAS_STATUS_SUCCESS) {
        printf("test_skip: cublasXtSetCpuRatio returned %s (Xt not fully supported on DTK)\n",
               UPTKblasGetStatusString(status));
    ...
    UPTKblasXtDestroy(handle);
    printf("test_cublasXtSetCpuRatio PASS\n");
    return 0;
}
