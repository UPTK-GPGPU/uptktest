...
int main() {
    ...
    double hx[4]={1,2,3,4};
    double ha=1.0;
    double *dx=NULL,*dAP=NULL;
    UPTKMalloc(&dx,4*sizeof(double));UPTKMalloc(&dAP,10*sizeof(double));
    UPTKMemcpy(dx,hx,4*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemset(dAP,0,10*sizeof(double));
    CHECK_CUBLAS(UPTKblasDspr(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dAP));
    UPTKFree(dx);UPTKFree(dAP);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDspr_v2 PASS\n");
    return 0;
}
