...
int main() {
    ...
    int n=4,k=4;
    double hA[16],hC[16];
    for(int i=0;i<16;i++){hA[i]=(double)(i+1);hC[i]=0;}
    double ha=1.0,hb=0.0;
    double *dA=NULL,*dC=NULL;
    UPTKMalloc(&dA,16*sizeof(double));UPTKMalloc(&dC,16*sizeof(double));
    UPTKMemcpy(dA,hA,16*sizeof(double),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dC,hC,16*sizeof(double),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasDsyrk(handle,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,n,k,&ha,dA,n,&hb,dC,n));
    UPTKFree(dA);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDsyrk_v2 PASS\n");
    return 0;
}
