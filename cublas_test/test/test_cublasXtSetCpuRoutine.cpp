...
int main() {
    ...
    status = UPTKblasXtSetCpuRoutine(handle, UPTKBLASXT_GEMM, UPTKBLASXT_FLOAT, NULL);
    if (status != UPTKBLAS_STATUS_SUCCESS) {
        printf("test_skip: cublasXtSetCpuRoutine returned %s (Xt not fully supported on DTK)\n",
               UPTKblasGetStatusString(status));
    ...
    UPTKblasXtDestroy(handle);
    printf("test_cublasXtSetCpuRoutine PASS\n");
    return 0;
}
