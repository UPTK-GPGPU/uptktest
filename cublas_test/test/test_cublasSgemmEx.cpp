...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasSgemmEx(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,UPTK_R_32F,m,dB,UPTK_R_32F,m,&hb,dC,UPTK_R_32F,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSgemmEx PASS\n");
    return 0;
}
