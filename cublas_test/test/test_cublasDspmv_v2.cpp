...
int main() {
    ...
    double *dAP=NULL,*dx=NULL,*dy=NULL;
    UPTKMalloc(&dAP,10*sizeof(double));UPTKMalloc(&dx,4*sizeof(double));UPTKMalloc(&dy,4*sizeof(double));
    ...
    CHECK_CUBLAS(UPTKblasDspmv(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dAP,dx,1,&hb,dy,1));
    UPTKFree(dAP);UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDspmv_v2 PASS\n");
    return 0;
}
