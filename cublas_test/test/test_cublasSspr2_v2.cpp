...
int main() {
    ...
    float ha=1.0f;
    float *dx=NULL,*dy=NULL,*dAP=NULL;
    UPTKMalloc(&dx,4*sizeof(float));UPTKMalloc(&dy,4*sizeof(float));UPTKMalloc(&dAP,10*sizeof(float));
    ...
    CHECK_CUBLAS(UPTKblasSspr2(handle,UPTKBLAS_FILL_MODE_LOWER,n,&ha,dx,1,dy,1,dAP));
    UPTKFree(dx);UPTKFree(dy);UPTKFree(dAP);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasSspr2_v2 PASS\n");
    return 0;
}
