...
int main() {
    ...
    cuDoubleComplex hA[32],hx[8],hy[8];
    ...
    UPTKMalloc(&dA,32*sizeof(cuDoubleComplex));UPTKMalloc(&dx,8*sizeof(cuDoubleComplex));UPTKMalloc(&dy,8*sizeof(cuDoubleComplex));
    UPTKMemcpy(dA,hA,32*sizeof(cuDoubleComplex),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dx,hx,8*sizeof(cuDoubleComplex),UPTKMemcpyHostToDevice);
    UPTKMemcpy(dy,hy,8*sizeof(cuDoubleComplex),UPTKMemcpyHostToDevice);
    CHECK_CUBLAS(UPTKblasZgemvStridedBatched(handle,UPTKBLAS_OP_N,m,n,&ha,dA,m,16,dx,1,4,&hb,dy,1,4,batchCount));
    UPTKFree(dA);UPTKFree(dx);UPTKFree(dy);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZgemvStridedBatched PASS\n");
    return 0;
}
