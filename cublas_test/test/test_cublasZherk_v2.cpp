...
int main() {
    ...
    int n=4,k=4;
    cuDoubleComplex hA[16],hC[16];
    for(int i=0;i<16;i++){hA[i].x=(float)(i+1);hA[i].y=0;hC[i].x=0;hC[i].y=0;}
    double ha=1.0,hb=0.0;
    cuDoubleComplex *dA=NULL,*dC=NULL;
    UPTKMalloc(&dA,16*sizeof(cuDoubleComplex));UPTKMalloc(&dC,16*sizeof(cuDoubleComplex));
    ...
    CHECK_CUBLAS(UPTKblasZherk(handle,UPTKBLAS_FILL_MODE_LOWER,UPTKBLAS_OP_N,n,k,&ha,dA,n,&hb,dC,n));
    UPTKFree(dA);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasZherk_v2 PASS\n");
    return 0;
}
