...
int main() {
    ...
    for(int i=0;i<16;i++){hA[i]=(double)(i+1);hB[i]=(double)(i+5);hC[i]=0;}
    double ha=1.0,hb=0.5;
    ...
    CHECK_CUBLAS(UPTKblasDgeam(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    CHECK_CUBLAS(UPTKblasDgeam(handle,UPTKBLAS_OP_T,UPTKBLAS_OP_N,m,n,&ha,dA,m,&hb,dB,m,dC,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasDgeam PASS\n");
    return 0;
}
