...
int main() {
    ...
    CHECK_CUBLAS(UPTKblasCgemmEx(handle,UPTKBLAS_OP_N,UPTKBLAS_OP_N,m,n,k,&ha,dA,UPTK_C_32F,m,dB,UPTK_C_32F,m,&hb,dC,UPTK_C_32F,m));
    UPTKFree(dA);UPTKFree(dB);UPTKFree(dC);
    CHECK_CUBLAS(UPTKblasDestroy(handle));
    printf("test_cublasCgemmEx PASS\n");
    return 0;
}
