/*
 * Auto-generated smoke test for driver wrapper UPArrayGetMemoryRequirements (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPArrayGetMemoryRequirements setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPArrayGetMemoryRequirements(&local_memoryRequirements, arr, dev);

    printf("UPArrayGetMemoryRequirements -> %d\n", (int)err);
    ...
    printf("test_UPArrayGetMemoryRequirements PASS\n");
    return 0;
}
