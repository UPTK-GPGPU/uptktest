/*
 * Auto-generated smoke test for driver wrapper UPOccupancyMaxActiveClusters (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPOccupancyMaxActiveClusters setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPOccupancyMaxActiveClusters(&local_numClusters, kern, &local_config);

    printf("UPOccupancyMaxActiveClusters -> %d\n", (int)err);
    ...
    printf("test_UPOccupancyMaxActiveClusters PASS\n");
    return 0;
}
