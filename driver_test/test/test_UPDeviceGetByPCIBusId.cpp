/*
 * Auto-generated smoke test for driver wrapper UPDeviceGetByPCIBusId (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPDeviceGetByPCIBusId setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPDeviceGetByPCIBusId(&local_dev, &local_pciBusId);

    printf("UPDeviceGetByPCIBusId -> %d\n", (int)err);
    ...
    printf("test_UPDeviceGetByPCIBusId PASS\n");
    return 0;
}
