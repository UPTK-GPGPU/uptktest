/*
 * Auto-generated smoke test for driver wrapper UPStreamWaitEvent (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPStreamWaitEvent setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPStreamWaitEvent(stream, evt, 0u);

    printf("UPStreamWaitEvent -> %d\n", (int)err);
    ...
    printf("test_UPStreamWaitEvent PASS\n");
    return 0;
}
