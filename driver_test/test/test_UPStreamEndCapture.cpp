/*
 * Auto-generated smoke test for driver wrapper UPStreamEndCapture (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPStreamEndCapture setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPStreamEndCapture(stream, &local_phGraph);

    printf("UPStreamEndCapture -> %d\n", (int)err);
    ...
    printf("test_UPStreamEndCapture PASS\n");
    return 0;
}
