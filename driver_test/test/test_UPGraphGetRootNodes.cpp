/*
 * Auto-generated smoke test for driver wrapper UPGraphGetRootNodes (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    size_t local_numRootNodes{};
    UPTKGraphNode_t local_rootNodes{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphGetRootNodes setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphGetRootNodes(graph, &local_rootNodes, &local_numRootNodes);

    printf("UPGraphGetRootNodes -> %d\n", (int)err);
    ...
    printf("test_UPGraphGetRootNodes PASS\n");
    return 0;
}
