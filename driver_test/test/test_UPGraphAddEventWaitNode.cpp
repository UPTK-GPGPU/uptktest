/*
 * Auto-generated smoke test for driver wrapper UPGraphAddEventWaitNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddEventWaitNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddEventWaitNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, evt);

    printf("UPGraphAddEventWaitNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddEventWaitNode PASS\n");
    return 0;
}
