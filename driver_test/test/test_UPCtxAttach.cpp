/*
 * Auto-generated smoke test for driver wrapper UPCtxAttach (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPCtxAttach setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPCtxAttach(&local_pctx, 0u);

    printf("UPCtxAttach -> %d\n", (int)err);
