/*
 * Auto-generated smoke test for driver wrapper UPStreamWaitValue64 (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPStreamWaitValue64 setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPStreamWaitValue64(stream, dptr, (cuuint64_t)0, 0u);

    printf("UPStreamWaitValue64 -> %d\n", (int)err);
    ...
    printf("test_UPStreamWaitValue64 PASS\n");
    return 0;
}
