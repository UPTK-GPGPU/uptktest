/*
 * Auto-generated smoke test for driver wrapper UPSignalExternalSemaphoresAsync (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPSignalExternalSemaphoresAsync setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPSignalExternalSemaphoresAsync(&local_extSemArray, &local_paramsArray, 0u, stream);

    printf("UPSignalExternalSemaphoresAsync -> %d\n", (int)err);
    ...
    printf("test_UPSignalExternalSemaphoresAsync PASS\n");
    return 0;
}
