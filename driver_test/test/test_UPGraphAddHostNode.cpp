/*
 * Auto-generated smoke test for driver wrapper UPGraphAddHostNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_HOST_NODE_PARAMS local_nodeParams{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddHostNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddHostNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_nodeParams);

    printf("UPGraphAddHostNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddHostNode PASS\n");
    return 0;
}
