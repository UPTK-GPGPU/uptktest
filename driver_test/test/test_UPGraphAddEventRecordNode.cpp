/*
 * Auto-generated smoke test for driver wrapper UPGraphAddEventRecordNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddEventRecordNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddEventRecordNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, evt);

    printf("UPGraphAddEventRecordNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddEventRecordNode PASS\n");
    return 0;
}
