/*
 * Auto-generated smoke test for driver wrapper UPGraphAddMemsetNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_MEMSET_NODE_PARAMS local_memsetParams{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddMemsetNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddMemsetNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_memsetParams, ctx);

    printf("UPGraphAddMemsetNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddMemsetNode PASS\n");
    return 0;
}
