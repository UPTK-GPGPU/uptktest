/*
 * Auto-generated smoke test for driver wrapper UPCtxCreate (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPCtxCreate setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPCtxCreate(&local_pctx, 0u, dev);

    printf("UPCtxCreate -> %d\n", (int)err);
