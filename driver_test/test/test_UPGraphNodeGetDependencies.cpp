/*
 * Auto-generated smoke test for driver wrapper UPGraphNodeGetDependencies (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphNodeGetDependencies setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphNodeGetDependencies(graphNode, &local_dependencies, &local_numDependencies);

    printf("UPGraphNodeGetDependencies -> %d\n", (int)err);
    ...
    printf("test_UPGraphNodeGetDependencies PASS\n");
    return 0;
}
