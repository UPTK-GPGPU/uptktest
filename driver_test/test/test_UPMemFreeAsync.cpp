/*
 * Auto-generated smoke test for driver wrapper UPMemFreeAsync (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMemFreeAsync setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPMemFreeAsync(dptr, stream);

    printf("UPMemFreeAsync -> %d\n", (int)err);
    ...
    printf("test_UPMemFreeAsync PASS\n");
    return 0;
}
