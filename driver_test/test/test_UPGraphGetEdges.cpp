/*
 * Auto-generated smoke test for driver wrapper UPGraphGetEdges (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphGetEdges setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphGetEdges(graph, &local_from, &local_to, &local_numEdges);

    printf("UPGraphGetEdges -> %d\n", (int)err);
    ...
    printf("test_UPGraphGetEdges PASS\n");
    return 0;
}
