/*
 * Auto-generated smoke test for driver wrapper UPGraphAddMemAllocNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_MEM_ALLOC_NODE_PARAMS local_nodeParams{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddMemAllocNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddMemAllocNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_nodeParams);

    printf("UPGraphAddMemAllocNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddMemAllocNode PASS\n");
    return 0;
}
