/*
 * Auto-generated smoke test for driver wrapper UPMemGetAccess (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMemGetAccess setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPMemGetAccess(&local_flags, &local_location, dptr);

    printf("UPMemGetAccess -> %d\n", (int)err);
    ...
    printf("test_UPMemGetAccess PASS\n");
    return 0;
}
