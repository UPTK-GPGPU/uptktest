/*
 * Auto-generated smoke test for driver wrapper UPCtxSetSharedMemConfig (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPCtxSetSharedMemConfig setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPCtxSetSharedMemConfig((UPTKsharedconfig)0);

    printf("UPCtxSetSharedMemConfig -> %d\n", (int)err);
    ...
    printf("test_UPCtxSetSharedMemConfig PASS\n");
    return 0;
}
