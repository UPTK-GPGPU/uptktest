/*
 * Auto-generated smoke test for driver wrapper UPGraphAddExternalSemaphoresSignalNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTKGraphNode_t local_dependencies{};
    UPTKGraphNode_t local_phGraphNode{};

    const int graph_level = 0;
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddExternalSemaphoresSignalNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddExternalSemaphoresSignalNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_nodeParams);

    printf("UPGraphAddExternalSemaphoresSignalNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddExternalSemaphoresSignalNode PASS\n");
    return 0;
}
