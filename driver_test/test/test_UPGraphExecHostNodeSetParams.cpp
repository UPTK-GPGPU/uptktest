/*
 * Auto-generated smoke test for driver wrapper UPGraphExecHostNodeSetParams (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphExecHostNodeSetParams setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphExecHostNodeSetParams(graphExec, graphNode, &local_nodeParams);

    printf("UPGraphExecHostNodeSetParams -> %d\n", (int)err);
    ...
    printf("test_UPGraphExecHostNodeSetParams PASS\n");
    return 0;
}
