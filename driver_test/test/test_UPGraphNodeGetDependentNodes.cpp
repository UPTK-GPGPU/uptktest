/*
 * Auto-generated smoke test for driver wrapper UPGraphNodeGetDependentNodes (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    size_t local_numDependentNodes{};
    UPTKGraphNode_t local_dependentNodes{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphNodeGetDependentNodes setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphNodeGetDependentNodes(graphNode, &local_dependentNodes, &local_numDependentNodes);

    printf("UPGraphNodeGetDependentNodes -> %d\n", (int)err);
    ...
    printf("test_UPGraphNodeGetDependentNodes PASS\n");
    return 0;
}
