/*
 * Auto-generated smoke test for driver wrapper UPStreamIsCapturing_ptsz (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTKStreamCaptureStatus local_captureStatus{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPStreamIsCapturing_ptsz setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPStreamIsCapturing_ptsz(stream, &local_captureStatus);

    printf("UPStreamIsCapturing_ptsz -> %d\n", (int)err);
    ...
    printf("test_UPStreamIsCapturing_ptsz PASS\n");
    return 0;
}
