/*
 * Auto-generated smoke test for driver wrapper UPGraphExecMemsetNodeSetParams (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphExecMemsetNodeSetParams setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphExecMemsetNodeSetParams(graphExec, graphNode, &local_memsetParams, ctx);

    printf("UPGraphExecMemsetNodeSetParams -> %d\n", (int)err);
    ...
    printf("test_UPGraphExecMemsetNodeSetParams PASS\n");
    return 0;
}
