/*
 * Auto-generated smoke test for driver wrapper UPMemFreeAsync_ptsz (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMemFreeAsync_ptsz setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPMemFreeAsync_ptsz(dptr, stream);

    printf("UPMemFreeAsync_ptsz -> %d\n", (int)err);
    ...
    printf("test_UPMemFreeAsync_ptsz PASS\n");
    return 0;
}
