/*
 * Auto-generated smoke test for driver wrapper UPGraphAddMemFreeNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddMemFreeNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddMemFreeNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, dptr);

    printf("UPGraphAddMemFreeNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddMemFreeNode PASS\n");
    return 0;
}
