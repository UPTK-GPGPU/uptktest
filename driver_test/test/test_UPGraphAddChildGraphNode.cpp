/*
 * Auto-generated smoke test for driver wrapper UPGraphAddChildGraphNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddChildGraphNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddChildGraphNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, graph);

    printf("UPGraphAddChildGraphNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddChildGraphNode PASS\n");
    return 0;
}
