/*
 * Auto-generated smoke test for driver wrapper UPLinkCreate (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPLinkCreate setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPLinkCreate(0u, &local_options, (void**)nullptr, &local_stateOut);

    printf("UPLinkCreate -> %d\n", (int)err);
