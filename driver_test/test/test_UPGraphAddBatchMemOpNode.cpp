/*
 * Auto-generated smoke test for driver wrapper UPGraphAddBatchMemOpNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_BATCH_MEM_OP_NODE_PARAMS local_nodeParams{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddBatchMemOpNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddBatchMemOpNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_nodeParams);

    printf("UPGraphAddBatchMemOpNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddBatchMemOpNode PASS\n");
    return 0;
}
