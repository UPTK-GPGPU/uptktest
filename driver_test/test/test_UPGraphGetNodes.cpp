/*
 * Auto-generated smoke test for driver wrapper UPGraphGetNodes (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    size_t local_numNodes{};
    UPTKGraphNode_t local_nodes{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphGetNodes setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphGetNodes(graph, &local_nodes, &local_numNodes);

    printf("UPGraphGetNodes -> %d\n", (int)err);
    ...
    printf("test_UPGraphGetNodes PASS\n");
    return 0;
}
