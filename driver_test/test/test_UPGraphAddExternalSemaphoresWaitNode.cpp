/*
 * Auto-generated smoke test for driver wrapper UPGraphAddExternalSemaphoresWaitNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddExternalSemaphoresWaitNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddExternalSemaphoresWaitNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_nodeParams);

    printf("UPGraphAddExternalSemaphoresWaitNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddExternalSemaphoresWaitNode PASS\n");
    return 0;
}
