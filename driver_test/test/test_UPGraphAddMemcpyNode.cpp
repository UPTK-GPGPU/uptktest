/*
 * Auto-generated smoke test for driver wrapper UPGraphAddMemcpyNode (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_MEMCPY3D local_copyParams{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphAddMemcpyNode setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphAddMemcpyNode(&local_phGraphNode, graph, &local_dependencies, (size_t)0, &local_copyParams, ctx);

    printf("UPGraphAddMemcpyNode -> %d\n", (int)err);
    ...
    printf("test_UPGraphAddMemcpyNode PASS\n");
    return 0;
}
