/*
 * Auto-generated smoke test for driver wrapper UPGraphExecMemcpyNodeSetParams (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPGraphExecMemcpyNodeSetParams setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPGraphExecMemcpyNodeSetParams(graphExec, graphNode, &local_copyParams, ctx);

    printf("UPGraphExecMemcpyNodeSetParams -> %d\n", (int)err);
    ...
    printf("test_UPGraphExecMemcpyNodeSetParams PASS\n");
    return 0;
}
