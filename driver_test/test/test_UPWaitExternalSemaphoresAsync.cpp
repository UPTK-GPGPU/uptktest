/*
 * Auto-generated smoke test for driver wrapper UPWaitExternalSemaphoresAsync (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPWaitExternalSemaphoresAsync setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPWaitExternalSemaphoresAsync(&local_extSemArray, &local_paramsArray, 0u, stream);

    printf("UPWaitExternalSemaphoresAsync -> %d\n", (int)err);
    ...
    printf("test_UPWaitExternalSemaphoresAsync PASS\n");
    return 0;
}
