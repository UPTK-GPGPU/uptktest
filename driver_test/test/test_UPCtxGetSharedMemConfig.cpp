/*
 * Auto-generated smoke test for driver wrapper UPCtxGetSharedMemConfig (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTKsharedconfig local_pConfig{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPCtxGetSharedMemConfig setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPCtxGetSharedMemConfig(&local_pConfig);

    printf("UPCtxGetSharedMemConfig -> %d\n", (int)err);
    ...
    printf("test_UPCtxGetSharedMemConfig PASS\n");
    return 0;
}
