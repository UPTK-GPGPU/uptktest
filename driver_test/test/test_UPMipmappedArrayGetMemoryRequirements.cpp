/*
 * Auto-generated smoke test for driver wrapper UPMipmappedArrayGetMemoryRequirements (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTK_ARRAY_MEMORY_REQUIREMENTS local_memoryRequirements{};

    const int graph_level = -1;
    const bool need_evt = false;
    const bool need_arr = false;
    const bool need_mipmap = true;
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMipmappedArrayGetMemoryRequirements setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPMipmappedArrayGetMemoryRequirements(&local_memoryRequirements, mipmap, dev);

    printf("UPMipmappedArrayGetMemoryRequirements -> %d\n", (int)err);
    ...
    printf("test_UPMipmappedArrayGetMemoryRequirements PASS\n");
    return 0;
}
