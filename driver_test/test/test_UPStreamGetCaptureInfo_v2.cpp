/*
 * Auto-generated smoke test for driver wrapper UPStreamGetCaptureInfo_v2 (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPStreamGetCaptureInfo_v2 setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPStreamGetCaptureInfo_v2(stream, &local_captureStatus_out, &local_id_out, &local_graph_out, &local_dependencies_out, &local_numDependencies_out);

    printf("UPStreamGetCaptureInfo_v2 -> %d\n", (int)err);
    ...
    printf("test_UPStreamGetCaptureInfo_v2 PASS\n");
    return 0;
}
