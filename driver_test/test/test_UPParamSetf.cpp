/*
 * Auto-generated smoke test for driver wrapper UPParamSetf (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPParamSetf setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPParamSetf(kern, 0, 0.f);

    printf("UPParamSetf -> %d\n", (int)err);
