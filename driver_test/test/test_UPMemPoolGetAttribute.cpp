/*
 * Auto-generated smoke test for driver wrapper UPMemPoolGetAttribute (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMemPoolGetAttribute setup failed (%d)\n", (int)err);
        return 0;
    }

    if (!memPool) {
        printf("test_skip: UPMemPoolGetAttribute needs valid mempool\n");
    } else {
        err = UPMemPoolGetAttribute(memPool, (UPTKmemPool_attribute)0, (void*)nullptr);
        printf("UPMemPoolGetAttribute -> %d\n", (int)err);
    }
    ...
    printf("test_UPMemPoolGetAttribute PASS\n");
    return 0;
}
