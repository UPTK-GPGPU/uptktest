/*
 * Auto-generated smoke test for driver wrapper UPMemPoolGetAccess (driver_fun_convert.cpp).
...
int main(void)
{
    ...
    UPTKmemAccess_flags local_flags{};
    UPTKMemLocation local_location{};
    ...
    if (err != UPTKSuccess) {
        printf("test_skip: UPMemPoolGetAccess setup failed (%d)\n", (int)err);
        return 0;
    }

    err = UPMemPoolGetAccess(&local_flags, memPool, &local_location);

    printf("UPMemPoolGetAccess -> %d\n", (int)err);
    ...
    printf("test_UPMemPoolGetAccess PASS\n");
    return 0;
}
