...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtMakePlanMany requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Make plan for C2C
    UPTKfftHandle plan;
    ...
    // Scene 2: Make plan for R2C
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    CHECK_CUFFT(UPTKfftXtSetGPUs(plan2, 2, gpus));
    CHECK_CUFFT(UPTKfftXtMakePlanMany(plan2, 1, n, NULL, 1, 0, UPTK_R_32F, NULL, 1, 0, UPTK_C_32F, 1, &workSize, UPTK_C_32F));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Make plan for Z2Z
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftCreate(&plan3));
    CHECK_CUFFT(UPTKfftXtSetGPUs(plan3, 2, gpus));
    CHECK_CUFFT(UPTKfftXtMakePlanMany(plan3, 1, n, NULL, 1, 0, UPTK_C_64F, NULL, 1, 0, UPTK_C_64F, 1, &workSize, UPTK_C_64F));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftXtMakePlanMany PASS\n");
    return 0;
}
