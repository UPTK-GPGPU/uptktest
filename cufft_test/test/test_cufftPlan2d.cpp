...
int main() {
    ...
    // Scene 1: Plan 2D C2C
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftPlan2d(&plan, 64, 64, UPTKFFT_C2C));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Plan 2D R2C
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftPlan2d(&plan2, 128, 64, UPTKFFT_R2C));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Plan 2D Z2Z
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftPlan2d(&plan3, 32, 32, UPTKFFT_Z2Z));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftPlan2d PASS\n");
    return 0;
}
