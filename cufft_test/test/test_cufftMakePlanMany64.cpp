...
int main() {
    ...
    // Scene 1: Make many 1D C2C plan (64-bit)
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    long long int n[] = {256};
    size_t workSize;
    CHECK_CUFFT(UPTKfftMakePlanMany64(plan, 1, n, NULL, 1, 0, NULL, 1, 0, UPTKFFT_C2C, 1, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Make many 2D R2C plan (64-bit)
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    long long int n2[] = {64, 64};
    CHECK_CUFFT(UPTKfftMakePlanMany64(plan2, 2, n2, NULL, 1, 0, NULL, 1, 0, UPTKFFT_R2C, 1, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Make many 1D Z2Z plan with batch (64-bit)
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftCreate(&plan3));
    long long int n3[] = {128};
    CHECK_CUFFT(UPTKfftMakePlanMany64(plan3, 1, n3, NULL, 1, 0, NULL, 1, 0, UPTKFFT_Z2Z, 5, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftMakePlanMany64 PASS\n");
    return 0;
}
