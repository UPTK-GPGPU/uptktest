...
int main() {
    ...
    // UPTKfftXtExec requires multi-GPU setup
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtExec requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Basic Xt exec with C2C
    UPTKfftHandle plan;
    ...
    UPTKMalloc((void**)&d_idata, 256 * sizeof(UPTKfftComplex));
    UPTKMalloc((void**)&d_odata, 256 * sizeof(UPTKfftComplex));

    UPTKfftResult res = UPTKfftXtExec(plan, d_idata, d_odata, CUFFT_FORWARD);
    if (res != CUFFT_SUCCESS) {
        printf("test_skip: UPTKfftXtExec not supported on this platform (DTK/AMD GPU)\n");
        UPTKFree(d_idata);
        UPTKFree(d_odata);
        UPTKfftDestroy(plan);
        return 0;
