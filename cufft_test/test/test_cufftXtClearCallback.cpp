...
int main() {
    ...
    // UPTKfftXtClearCallback requires multi-GPU setup
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtClearCallback requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Create plan, set GPUs, clear callback
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    int gpus[] = {0, 1};
    CHECK_CUFFT(UPTKfftXtSetGPUs(plan, 2, gpus));
    CHECK_CUFFT(UPTKfftXtClearCallback(plan, UPTKFFT_CB_LD_COMPLEX));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftXtClearCallback PASS\n");
    return 0;
}
