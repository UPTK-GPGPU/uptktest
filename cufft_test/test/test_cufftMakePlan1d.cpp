...
int main() {
    ...
    // Scene 1: Make 1D C2C plan
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    size_t workSize;
    CHECK_CUFFT(UPTKfftMakePlan1d(plan, 256, UPTKFFT_C2C, 1, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Make 1D R2C plan
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    CHECK_CUFFT(UPTKfftMakePlan1d(plan2, 512, UPTKFFT_R2C, 1, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Make 1D Z2Z plan with batch
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftCreate(&plan3));
    CHECK_CUFFT(UPTKfftMakePlan1d(plan3, 128, UPTKFFT_Z2Z, 5, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftMakePlan1d PASS\n");
    return 0;
}
