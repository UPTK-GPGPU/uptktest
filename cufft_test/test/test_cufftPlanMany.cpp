...
int main() {
    ...
    // Scene 1: Plan many 1D C2C
    UPTKfftHandle plan;
    int n[] = {256};
    CHECK_CUFFT(UPTKfftPlanMany(&plan, 1, n, NULL, 1, 0, NULL, 1, 0, UPTKFFT_C2C, 1));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Plan many 2D R2C
    UPTKfftHandle plan2;
    int n2[] = {64, 64};
    CHECK_CUFFT(UPTKfftPlanMany(&plan2, 2, n2, NULL, 1, 0, NULL, 1, 0, UPTKFFT_R2C, 1));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Plan many 1D Z2Z with batch
    UPTKfftHandle plan3;
    int n3[] = {128};
    CHECK_CUFFT(UPTKfftPlanMany(&plan3, 1, n3, NULL, 1, 0, NULL, 1, 0, UPTKFFT_Z2Z, 5));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftPlanMany PASS\n");
    return 0;
}
