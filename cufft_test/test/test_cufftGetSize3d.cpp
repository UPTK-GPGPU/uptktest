...
int main() {
    ...
    // Scene 1: Get size for 3D C2C
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    size_t workSize;
    CHECK_CUFFT(UPTKfftGetSize3d(plan, 32, 32, 32, UPTKFFT_C2C, &workSize));

    // Scene 2: Get size for 3D R2C
    CHECK_CUFFT(UPTKfftGetSize3d(plan, 64, 32, 16, UPTKFFT_R2C, &workSize));

    // Scene 3: Get size for 3D Z2Z
    CHECK_CUFFT(UPTKfftGetSize3d(plan, 16, 16, 16, UPTKFFT_Z2Z, &workSize));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftGetSize3d PASS\n");
    return 0;
}
