...
int main() {
    ...
    // Scene 1: Get size for 1D C2C
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    size_t workSize;
    CHECK_CUFFT(UPTKfftGetSize1d(plan, 256, UPTKFFT_C2C, 1, &workSize));

    // Scene 2: Get size for 1D R2C
    CHECK_CUFFT(UPTKfftGetSize1d(plan, 512, UPTKFFT_R2C, 1, &workSize));

    // Scene 3: Get size for 1D Z2Z with batch
    CHECK_CUFFT(UPTKfftGetSize1d(plan, 128, UPTKFFT_Z2Z, 5, &workSize));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftGetSize1d PASS\n");
    return 0;
}
