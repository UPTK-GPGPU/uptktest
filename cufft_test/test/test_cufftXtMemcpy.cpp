...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtMemcpy requires multiple GPUs\n");
        return 0;
    }
    ...
    UPTKfftComplex h_data[256] = {0};
    UPTKLibXtDesc *d_desc;
    CHECK_CUFFT(UPTKfftXtMalloc(plan, &d_desc, UPTKFFT_XT_FORMAT_INPUT));
    CHECK_CUFFT(UPTKfftXtMemcpy(plan, d_desc, h_data, UPTKFFT_COPY_HOST_TO_DEVICE));
    CHECK_CUFFT(UPTKfftXtFree(d_desc));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftXtMemcpy PASS\n");
    return 0;
}
