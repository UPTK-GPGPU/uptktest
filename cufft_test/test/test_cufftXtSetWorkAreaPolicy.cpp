...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtSetWorkAreaPolicy requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Set MINIMAL policy
    UPTKfftHandle plan;
    ...
    size_t workSize = 0;
    UPTKfftResult res = UPTKfftXtSetWorkAreaPolicy(plan, UPTKFFT_WORKAREA_MINIMAL, &workSize);
    if (res != CUFFT_SUCCESS && res != CUFFT_NOT_IMPLEMENTED && res != CUFFT_NOT_SUPPORTED) {
    ...
    // Scene 2: Set PERFORMANCE policy
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    CHECK_CUFFT(UPTKfftXtSetGPUs(plan2, 2, gpus));
    res = UPTKfftXtSetWorkAreaPolicy(plan2, UPTKFFT_WORKAREA_PERFORMANCE, &workSize);
    if (res != CUFFT_SUCCESS && res != CUFFT_NOT_IMPLEMENTED && res != CUFFT_NOT_SUPPORTED) {
        printf("cuFFT error: %s\n", cufftGetErrorString(res));
        return 1;
    }
    ...
    printf("test_cufftXtSetWorkAreaPolicy PASS\n");
    return 0;
}
