...
int main() {
    ...
    // Scene 1: Get size from a 1D plan
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftPlan1d(&plan, 256, UPTKFFT_C2C, 1));
    size_t workSize;
    CHECK_CUFFT(UPTKfftGetSize(plan, &workSize));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Get size from a 2D plan
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftPlan2d(&plan2, 64, 64, UPTKFFT_C2C));
    CHECK_CUFFT(UPTKfftGetSize(plan2, &workSize));

    CHECK_CUFFT(UPTKfftDestroy(plan2));

    printf("test_cufftGetSize PASS\n");
    return 0;
}
