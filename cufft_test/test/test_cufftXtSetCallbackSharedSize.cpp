...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtSetCallbackSharedSize requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Set shared size for callback
    UPTKfftHandle plan;
    ...
    UPTKfftResult res = UPTKfftXtSetCallbackSharedSize(plan, UPTKFFT_CB_LD_COMPLEX, 1024);
    if (res != CUFFT_SUCCESS && res != CUFFT_NOT_IMPLEMENTED && res != CUFFT_INVALID_VALUE) {
    ...
    printf("test_cufftXtSetCallbackSharedSize PASS\n");
    return 0;
}
