...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtSetWorkArea requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Set work area
    UPTKfftHandle plan;
    ...
    void *workArea = d_work;
    UPTKfftResult res = UPTKfftXtSetWorkArea(plan, &workArea);
    UPTKFree(d_work);
    if (res != CUFFT_SUCCESS && res != CUFFT_INTERNAL_ERROR) {
    ...
    printf("test_cufftXtSetWorkArea PASS\n");
    return 0;
}
