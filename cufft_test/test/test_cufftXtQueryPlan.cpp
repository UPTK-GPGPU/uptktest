...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtQueryPlan requires multiple GPUs\n");
        return 0;
    }

    // Scene 1: Query 1D factors
    UPTKfftHandle plan;
    ...
    UPTKfftXt1dFactors factors;
    CHECK_CUFFT(UPTKfftXtQueryPlan(plan, &factors, UPTKFFT_QUERY_1D_FACTORS));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftXtQueryPlan PASS\n");
    return 0;
}
