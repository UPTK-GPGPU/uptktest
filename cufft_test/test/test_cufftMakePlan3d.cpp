...
int main() {
    ...
    // Scene 1: Make 3D C2C plan
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    size_t workSize;
    CHECK_CUFFT(UPTKfftMakePlan3d(plan, 32, 32, 32, UPTKFFT_C2C, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Make 3D R2C plan
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    CHECK_CUFFT(UPTKfftMakePlan3d(plan2, 64, 32, 16, UPTKFFT_R2C, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Make 3D Z2Z plan
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftCreate(&plan3));
    CHECK_CUFFT(UPTKfftMakePlan3d(plan3, 16, 16, 16, UPTKFFT_Z2Z, &workSize));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftMakePlan3d PASS\n");
    return 0;
}
