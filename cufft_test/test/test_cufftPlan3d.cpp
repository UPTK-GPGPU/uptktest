...
int main() {
    ...
    // Scene 1: Plan 3D C2C
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftPlan3d(&plan, 32, 32, 32, UPTKFFT_C2C));
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Plan 3D R2C
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftPlan3d(&plan2, 64, 32, 16, UPTKFFT_R2C));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    // Scene 3: Plan 3D Z2Z
    UPTKfftHandle plan3;
    CHECK_CUFFT(UPTKfftPlan3d(&plan3, 16, 16, 16, UPTKFFT_Z2Z));
    CHECK_CUFFT(UPTKfftDestroy(plan3));

    printf("test_cufftPlan3d PASS\n");
    return 0;
}
