...
int main() {
    ...
    if (deviceCount < 2) {
        printf("test_skip: UPTKfftXtSetGPUs requires multiple GPUs\n");
        return 0;
    }
    ...
    // Scene 2: Set different GPU combination
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftCreate(&plan2));
    int gpus2[] = {1, 0};
    CHECK_CUFFT(UPTKfftXtSetGPUs(plan2, 2, gpus2));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    printf("test_cufftXtSetGPUs PASS\n");
    return 0;
}
