...
int main() {
    ...
    // Scene 1: Get size for 2D C2C
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftCreate(&plan));
    size_t workSize;
    CHECK_CUFFT(UPTKfftGetSize2d(plan, 64, 64, UPTKFFT_C2C, &workSize));

    // Scene 2: Get size for 2D R2C
    CHECK_CUFFT(UPTKfftGetSize2d(plan, 128, 64, UPTKFFT_R2C, &workSize));

    // Scene 3: Get size for 2D Z2Z
    CHECK_CUFFT(UPTKfftGetSize2d(plan, 32, 32, UPTKFFT_Z2Z, &workSize));

    CHECK_CUFFT(UPTKfftDestroy(plan));

    printf("test_cufftGetSize2d PASS\n");
    return 0;
}
