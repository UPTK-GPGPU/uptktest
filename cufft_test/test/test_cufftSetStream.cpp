...
int main() {
    ...
    // Scene 1: Set custom stream
    UPTKfftHandle plan;
    CHECK_CUFFT(UPTKfftPlan1d(&plan, 256, UPTKFFT_C2C, 1));
    UPTKStream_t stream;
    UPTKStreamCreate(&stream);
    CHECK_CUFFT(UPTKfftSetStream(plan, stream));
    UPTKStreamDestroy(stream);
    CHECK_CUFFT(UPTKfftDestroy(plan));

    // Scene 2: Set NULL stream (default)
    UPTKfftHandle plan2;
    CHECK_CUFFT(UPTKfftPlan1d(&plan2, 256, UPTKFFT_C2C, 1));
    CHECK_CUFFT(UPTKfftSetStream(plan2, NULL));
    CHECK_CUFFT(UPTKfftDestroy(plan2));

    printf("test_cufftSetStream PASS\n");
    return 0;
}
