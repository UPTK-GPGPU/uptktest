/*
 * Auto-generated smoke test for UPTKsparseDnVecGetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnVecGetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnVecGetValues(dnVecDescr, (void**)nullptr);

    printf("UPTKsparseDnVecGetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDnVecGetValues PASS\n");
    return 0;
}
