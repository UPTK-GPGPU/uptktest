/*
 * Auto-generated smoke test for UPTKsparseCcsrilu02_numericBoost (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCcsrilu02_numericBoost setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCcsrilu02_numericBoost(sparse_handle, (csrilu02Info_t)0, 0, (double*)dev_scratch, (cuComplex*)dev_scratch);

    printf("UPTKsparseCcsrilu02_numericBoost -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCcsrilu02_numericBoost PASS\n");
    return 0;
}
