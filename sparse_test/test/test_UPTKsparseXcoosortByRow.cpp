/*
 * Auto-generated smoke test for UPTKsparseXcoosortByRow (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcoosortByRow setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcoosortByRow(sparse_handle, 0, 0, 0, (int*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, (void*)nullptr);

    printf("UPTKsparseXcoosortByRow -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXcoosortByRow PASS\n");
    return 0;
}
