/*
 * Auto-generated smoke test for UPTKsparseCbsrmm (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrmm setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrmm UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrmm SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrmm SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCbsrmm(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, 0, 0, (const cuComplex*)dev_scratch, descrA, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, (const cuComplex*)dev_scratch, 0, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, 0);

    printf("UPTKsparseCbsrmm -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCbsrmm PASS\n");
    return 0;
}
