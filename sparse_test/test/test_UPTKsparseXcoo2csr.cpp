/*
 * Auto-generated smoke test for UPTKsparseXcoo2csr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcoo2csr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcoo2csr(sparse_handle, (const int*)dev_scratch, 0, 0, (int*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO);

    printf("UPTKsparseXcoo2csr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXcoo2csr PASS\n");
    return 0;
}
