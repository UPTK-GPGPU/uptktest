/*
 * Auto-generated smoke test for UPTKsparseCgtsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgtsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgtsvInterleavedBatch(sparse_handle, 0, 0, (cuComplex*)dev_scratch, (cuComplex*)dev_scratch, (cuComplex*)dev_scratch, (cuComplex*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseCgtsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgtsvInterleavedBatch PASS\n");
    return 0;
}
