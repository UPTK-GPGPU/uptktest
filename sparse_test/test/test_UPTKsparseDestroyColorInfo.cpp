/*
 * Auto-generated smoke test for UPTKsparseDestroyColorInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyColorInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyColorInfo(info);

    printf("UPTKsparseDestroyColorInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyColorInfo PASS\n");
    return 0;
}
