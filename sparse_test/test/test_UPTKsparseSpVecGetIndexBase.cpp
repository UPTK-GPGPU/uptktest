/*
 * Auto-generated smoke test for UPTKsparseSpVecGetIndexBase (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpVecGetIndexBase setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpVecGetIndexBase(spVecDescr, (UPTKsparseIndexBase_t*)dev_scratch);

    printf("UPTKsparseSpVecGetIndexBase -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpVecGetIndexBase PASS\n");
    return 0;
}
