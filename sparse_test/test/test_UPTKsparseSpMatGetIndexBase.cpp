/*
 * Auto-generated smoke test for UPTKsparseSpMatGetIndexBase (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatGetIndexBase setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatGetIndexBase(spMatDescr, (UPTKsparseIndexBase_t*)dev_scratch);

    printf("UPTKsparseSpMatGetIndexBase -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMatGetIndexBase PASS\n");
    return 0;
}
