/*
 * Auto-generated smoke test for UPTKsparseDgpsvInterleavedBatch_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgpsvInterleavedBatch_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgpsvInterleavedBatch_bufferSizeExt(sparse_handle, 0, 0, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, 0, (size_t*)dev_scratch);

    printf("UPTKsparseDgpsvInterleavedBatch_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDgpsvInterleavedBatch_bufferSizeExt PASS\n");
    return 0;
}
