/*
 * Auto-generated smoke test for UPTKsparseDestroyCsrilu02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyCsrilu02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyCsrilu02Info((csrilu02Info_t)0);

    printf("UPTKsparseDestroyCsrilu02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyCsrilu02Info PASS\n");
    return 0;
}
