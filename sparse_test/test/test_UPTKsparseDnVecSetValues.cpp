/*
 * Auto-generated smoke test for UPTKsparseDnVecSetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnVecSetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnVecSetValues(dnVecDescr, (void*)nullptr);

    printf("UPTKsparseDnVecSetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnVecSetValues PASS\n");
    return 0;
}
