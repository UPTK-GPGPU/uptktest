/*
 * Auto-generated smoke test for UPTKsparseSpSV_destroyDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKsparseSpSVDescr_t descr{};

    UPTKsparseStatus_t err;

    err = sparse_setup_core(&sparse_handle, &dev_scratch, &stream_id, 1);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSV_destroyDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSV_destroyDescr(descr);

    printf("UPTKsparseSpSV_destroyDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpSV_destroyDescr PASS\n");
    return 0;
}
