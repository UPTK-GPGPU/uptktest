/*
 * Auto-generated smoke test for UPTKsparseSpVecGetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpVecGetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpVecGetValues(spVecDescr, (void**)nullptr);

    printf("UPTKsparseSpVecGetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpVecGetValues PASS\n");
    return 0;
}
