/*
 * Auto-generated smoke test for UPTKsparseDestroyBsrsm2Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyBsrsm2Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyBsrsm2Info((bsrsm2Info_t)0);

    printf("UPTKsparseDestroyBsrsm2Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDestroyBsrsm2Info PASS\n");
    return 0;
}
