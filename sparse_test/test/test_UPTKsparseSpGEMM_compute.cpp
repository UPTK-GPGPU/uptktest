/*
 * Auto-generated smoke test for UPTKsparseSpGEMM_compute (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpGEMM_compute setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpGEMM_compute(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, matB, (const void*)dev_scratch, matC, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpGEMMAlg_t)0, spgemmDescr, (size_t*)dev_scratch, (void*)nullptr);

    printf("UPTKsparseSpGEMM_compute -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpGEMM_compute PASS\n");
    return 0;
}
