/*
 * Auto-generated smoke test for UPTKsparseDgebsr2gebsc (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgebsr2gebsc setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgebsr2gebsc(sparse_handle, 0, 0, 0, (const double*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, (double*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, (UPTKsparseAction_t)0, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseDgebsr2gebsc -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDgebsr2gebsc PASS\n");
    return 0;
}
