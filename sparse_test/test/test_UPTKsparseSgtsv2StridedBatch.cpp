/*
 * Auto-generated smoke test for UPTKsparseSgtsv2StridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgtsv2StridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgtsv2StridedBatch(sparse_handle, 0, (const float*)dev_scratch, (const float*)dev_scratch, (const float*)dev_scratch, (float*)dev_scratch, 0, 0, (void*)nullptr);

    printf("UPTKsparseSgtsv2StridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSgtsv2StridedBatch PASS\n");
    return 0;
}
