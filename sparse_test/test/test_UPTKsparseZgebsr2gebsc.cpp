/*
 * Auto-generated smoke test for UPTKsparseZgebsr2gebsc (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsc setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgebsr2gebsc(sparse_handle, 0, 0, 0, (const cuDoubleComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, (cuDoubleComplex*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, (UPTKsparseAction_t)0, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseZgebsr2gebsc -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseZgebsr2gebsc PASS\n");
    return 0;
}
