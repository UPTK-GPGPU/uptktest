/*
 * Auto-generated smoke test for UPTKsparseSpGEMM_destroyDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKsparseSpGEMMDescr_t descr{};

    UPTKsparseStatus_t err;

    err = sparse_setup_core(&sparse_handle, &dev_scratch, &stream_id, 1);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpGEMM_destroyDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpGEMM_destroyDescr(descr);

    printf("UPTKsparseSpGEMM_destroyDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpGEMM_destroyDescr PASS\n");
    return 0;
}
