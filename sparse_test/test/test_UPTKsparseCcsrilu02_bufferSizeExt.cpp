/*
 * Auto-generated smoke test for UPTKsparseCcsrilu02_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCcsrilu02_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCcsrilu02_bufferSizeExt UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCcsrilu02_bufferSizeExt SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCcsrilu02_bufferSizeExt SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCcsrilu02_bufferSizeExt(sparse_handle, 0, 0, descrA, (cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, (csrilu02Info_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseCcsrilu02_bufferSizeExt -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCcsrilu02_bufferSizeExt PASS\n");
    return 0;
}
