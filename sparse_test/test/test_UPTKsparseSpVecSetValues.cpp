/*
 * Auto-generated smoke test for UPTKsparseSpVecSetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpVecSetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpVecSetValues(spVecDescr, (void*)nullptr);

    printf("UPTKsparseSpVecSetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpVecSetValues PASS\n");
    return 0;
}
