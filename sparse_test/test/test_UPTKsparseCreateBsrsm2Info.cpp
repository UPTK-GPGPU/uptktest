/*
 * Auto-generated smoke test for UPTKsparseCreateBsrsm2Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateBsrsm2Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateBsrsm2Info((bsrsm2Info_t*)dev_scratch);

    printf("UPTKsparseCreateBsrsm2Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateBsrsm2Info PASS\n");
    return 0;
}
