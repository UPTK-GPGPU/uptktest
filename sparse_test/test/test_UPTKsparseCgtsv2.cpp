/*
 * Auto-generated smoke test for UPTKsparseCgtsv2 (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgtsv2 setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgtsv2(sparse_handle, 0, 0, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseCgtsv2 -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgtsv2 PASS\n");
    return 0;
}
