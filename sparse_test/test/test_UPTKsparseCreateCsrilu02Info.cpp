/*
 * Auto-generated smoke test for UPTKsparseCreateCsrilu02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateCsrilu02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateCsrilu02Info((csrilu02Info_t*)dev_scratch);

    printf("UPTKsparseCreateCsrilu02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateCsrilu02Info PASS\n");
    return 0;
}
