/*
 * Auto-generated smoke test for UPTKsparseSpGEMMreuse_nnz (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpGEMMreuse_nnz setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpGEMMreuse_nnz(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, matA, matB, matC, (UPTKsparseSpGEMMAlg_t)0, spgemmDescr, (size_t*)dev_scratch, (void*)nullptr, (size_t*)dev_scratch, (void*)nullptr, (size_t*)dev_scratch, (void*)nullptr);

    printf("UPTKsparseSpGEMMreuse_nnz -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpGEMMreuse_nnz PASS\n");
    return 0;
}
