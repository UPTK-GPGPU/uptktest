/*
 * Auto-generated smoke test for UPTKsparseAxpby (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseAxpby setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseAxpby(sparse_handle, (const void*)dev_scratch, vecX, (const void*)dev_scratch, vecY);

    printf("UPTKsparseAxpby -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseAxpby PASS\n");
    return 0;
}
