/*
 * Auto-generated smoke test for UPTKsparseSpMMOp_destroyPlan (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMMOp_destroyPlan setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMMOp_destroyPlan(plan);

    printf("UPTKsparseSpMMOp_destroyPlan -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMMOp_destroyPlan PASS\n");
    return 0;
}
