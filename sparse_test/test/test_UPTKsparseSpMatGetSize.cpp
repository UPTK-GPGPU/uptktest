/*
 * Auto-generated smoke test for UPTKsparseSpMatGetSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatGetSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatGetSize(spMatDescr, (int64_t*)dev_scratch, (int64_t*)dev_scratch, (int64_t*)dev_scratch);

    printf("UPTKsparseSpMatGetSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMatGetSize PASS\n");
    return 0;
}
