/*
 * Auto-generated smoke test for UPTKsparseScsru2csr_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsru2csr_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseScsru2csr_bufferSizeExt(sparse_handle, 0, 0, 0, (float*)dev_scratch, (const int*)dev_scratch, (int*)dev_scratch, (csru2csrInfo_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseScsru2csr_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseScsru2csr_bufferSizeExt PASS\n");
    return 0;
}
