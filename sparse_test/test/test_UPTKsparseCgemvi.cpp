/*
 * Auto-generated smoke test for UPTKsparseCgemvi (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgemvi setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgemvi(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, 0, 0, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseCgemvi -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgemvi PASS\n");
    return 0;
}
