/*
 * Auto-generated smoke test for UPTKsparseCreateMatDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateMatDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateMatDescr((UPTKsparseMatDescr_t*)dev_scratch);

    printf("UPTKsparseCreateMatDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateMatDescr PASS\n");
    return 0;
}
