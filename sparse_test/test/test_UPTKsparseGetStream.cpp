/*
 * Auto-generated smoke test for UPTKsparseGetStream (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseGetStream setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseGetStream(sparse_handle, (UPTKStream_t*)dev_scratch);

    printf("UPTKsparseGetStream -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseGetStream PASS\n");
    return 0;
}
