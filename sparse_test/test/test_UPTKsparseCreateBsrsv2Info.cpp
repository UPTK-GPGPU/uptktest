/*
 * Auto-generated smoke test for UPTKsparseCreateBsrsv2Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateBsrsv2Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateBsrsv2Info((bsrsv2Info_t*)dev_scratch);

    printf("UPTKsparseCreateBsrsv2Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCreateBsrsv2Info PASS\n");
    return 0;
}
