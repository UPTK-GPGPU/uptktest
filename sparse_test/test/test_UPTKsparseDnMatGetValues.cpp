/*
 * Auto-generated smoke test for UPTKsparseDnMatGetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnMatGetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnMatGetValues(dnMatDescr, (void**)nullptr);

    printf("UPTKsparseDnMatGetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnMatGetValues PASS\n");
    return 0;
}
