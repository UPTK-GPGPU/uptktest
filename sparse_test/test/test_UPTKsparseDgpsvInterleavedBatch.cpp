/*
 * Auto-generated smoke test for UPTKsparseDgpsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgpsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgpsvInterleavedBatch(sparse_handle, 0, 0, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseDgpsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDgpsvInterleavedBatch PASS\n");
    return 0;
}
