/*
 * Auto-generated smoke test for UPTKsparseSpMatGetStridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatGetStridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatGetStridedBatch(spMatDescr, (int*)dev_scratch);

    printf("UPTKsparseSpMatGetStridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMatGetStridedBatch PASS\n");
    return 0;
}
