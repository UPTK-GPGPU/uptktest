/*
 * Auto-generated smoke test for UPTKsparseSparseToDense (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSparseToDense setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSparseToDense(sparse_handle, matA, matB, (UPTKsparseSparseToDenseAlg_t)0, (void*)nullptr);

    printf("UPTKsparseSparseToDense -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSparseToDense PASS\n");
    return 0;
}
