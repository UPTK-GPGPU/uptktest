/*
 * Auto-generated smoke test for UPTKsparseZgemvi_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgemvi_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgemvi_bufferSize(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, 0, (int*)dev_scratch);

    printf("UPTKsparseZgemvi_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseZgemvi_bufferSize PASS\n");
    return 0;
}
