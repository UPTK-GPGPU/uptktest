/*
 * Auto-generated smoke test for UPTKsparseSgtsv2_nopivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgtsv2_nopivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgtsv2_nopivot(sparse_handle, 0, 0, (const float*)dev_scratch, (const float*)dev_scratch, (const float*)dev_scratch, (float*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseSgtsv2_nopivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSgtsv2_nopivot PASS\n");
    return 0;
}
