/*
 * Auto-generated smoke test for UPTKsparseSgpsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgpsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgpsvInterleavedBatch(sparse_handle, 0, 0, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseSgpsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSgpsvInterleavedBatch PASS\n");
    return 0;
}
