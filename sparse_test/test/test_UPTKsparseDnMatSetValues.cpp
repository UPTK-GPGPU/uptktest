/*
 * Auto-generated smoke test for UPTKsparseDnMatSetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnMatSetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnMatSetValues(dnMatDescr, (void*)nullptr);

    printf("UPTKsparseDnMatSetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnMatSetValues PASS\n");
    return 0;
}
