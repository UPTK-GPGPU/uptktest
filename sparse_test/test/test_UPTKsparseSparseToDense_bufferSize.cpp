/*
 * Auto-generated smoke test for UPTKsparseSparseToDense_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSparseToDense_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSparseToDense_bufferSize(sparse_handle, matA, matB, (UPTKsparseSparseToDenseAlg_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseSparseToDense_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSparseToDense_bufferSize PASS\n");
    return 0;
}
