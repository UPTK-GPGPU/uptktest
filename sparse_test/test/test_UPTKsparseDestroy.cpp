/*
 * Auto-generated smoke test for UPTKsparseDestroy (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroy setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroy(sparse_handle);

    printf("UPTKsparseDestroy -> %d\n", (int)err);

    sparse_handle = (UPTKsparseHandle_t)(uintptr_t)0;

    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 0);
    printf("test_UPTKsparseDestroy PASS\n");
    return 0;
}
