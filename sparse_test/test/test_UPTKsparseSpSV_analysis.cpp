/*
 * Auto-generated smoke test for UPTKsparseSpSV_analysis (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSV_analysis setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSV_analysis(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, vecX, vecY, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpSVAlg_t)0, spsvDescr, (void*)nullptr);

    printf("UPTKsparseSpSV_analysis -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSV_analysis PASS\n");
    return 0;
}
