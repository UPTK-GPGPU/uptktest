/*
 * Auto-generated smoke test for UPTKsparseCreateBsric02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateBsric02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateBsric02Info((bsric02Info_t*)dev_scratch);

    printf("UPTKsparseCreateBsric02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateBsric02Info PASS\n");
    return 0;
}
