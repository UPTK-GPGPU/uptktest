/*
 * Auto-generated smoke test for UPTKsparseScsr2csr_compress (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsr2csr_compress setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsr2csr_compress UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsr2csr_compress SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsr2csr_compress SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseScsr2csr_compress(sparse_handle, 0, 0, descrA, (const float*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, (const int*)dev_scratch, (float*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, 0.f);

    printf("UPTKsparseScsr2csr_compress -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseScsr2csr_compress PASS\n");
    return 0;
}
