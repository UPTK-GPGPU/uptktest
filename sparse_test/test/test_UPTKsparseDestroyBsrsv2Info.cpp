/*
 * Auto-generated smoke test for UPTKsparseDestroyBsrsv2Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyBsrsv2Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyBsrsv2Info((bsrsv2Info_t)0);

    printf("UPTKsparseDestroyBsrsv2Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyBsrsv2Info PASS\n");
    return 0;
}
