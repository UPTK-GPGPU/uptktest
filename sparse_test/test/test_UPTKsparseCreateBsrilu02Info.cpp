/*
 * Auto-generated smoke test for UPTKsparseCreateBsrilu02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateBsrilu02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateBsrilu02Info((bsrilu02Info_t*)dev_scratch);

    printf("UPTKsparseCreateBsrilu02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCreateBsrilu02Info PASS\n");
    return 0;
}
