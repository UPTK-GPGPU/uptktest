/*
 * Auto-generated smoke test for UPTKsparseSgtsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgtsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgtsvInterleavedBatch(sparse_handle, 0, 0, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, (float*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseSgtsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSgtsvInterleavedBatch PASS\n");
    return 0;
}
