/*
 * Auto-generated smoke test for UPTKsparseDgtsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgtsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgtsvInterleavedBatch(sparse_handle, 0, 0, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, (double*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseDgtsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDgtsvInterleavedBatch PASS\n");
    return 0;
}
