/*
 * Auto-generated smoke test for UPTKsparseSpMatGetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatGetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatGetValues(spMatDescr, (void**)nullptr);

    printf("UPTKsparseSpMatGetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMatGetValues PASS\n");
    return 0;
}
