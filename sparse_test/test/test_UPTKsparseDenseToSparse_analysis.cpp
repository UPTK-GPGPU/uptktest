/*
 * Auto-generated smoke test for UPTKsparseDenseToSparse_analysis (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDenseToSparse_analysis setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDenseToSparse_analysis(sparse_handle, matA, matB, (UPTKsparseDenseToSparseAlg_t)0, (void*)nullptr);

    printf("UPTKsparseDenseToSparse_analysis -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDenseToSparse_analysis PASS\n");
    return 0;
}
