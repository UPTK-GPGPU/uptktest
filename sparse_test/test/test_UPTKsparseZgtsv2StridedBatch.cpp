/*
 * Auto-generated smoke test for UPTKsparseZgtsv2StridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgtsv2StridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgtsv2StridedBatch(sparse_handle, 0, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, 0, 0, (void*)nullptr);

    printf("UPTKsparseZgtsv2StridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseZgtsv2StridedBatch PASS\n");
    return 0;
}
