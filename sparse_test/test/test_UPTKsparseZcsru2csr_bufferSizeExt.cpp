/*
 * Auto-generated smoke test for UPTKsparseZcsru2csr_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZcsru2csr_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZcsru2csr_bufferSizeExt(sparse_handle, 0, 0, 0, (cuDoubleComplex*)dev_scratch, (const int*)dev_scratch, (int*)dev_scratch, (csru2csrInfo_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseZcsru2csr_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseZcsru2csr_bufferSizeExt PASS\n");
    return 0;
}
