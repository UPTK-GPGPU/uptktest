/*
 * Auto-generated smoke test for UPTKsparseCnnz_compress (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCnnz_compress UPTKsparseCreateMatDescr(descr) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descr, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCnnz_compress SetMatIndexBase(descr)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCnnz_compress SetMatType(descr)\n");
        UPTKsparseDestroyMatDescr(descr); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseCnnz_compress(sparse_handle, 0, descr, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, (cuComplex){0.f, 0.f});

    printf("UPTKsparseCnnz_compress -> %d\n", (int)err);



    if (descr) UPTKsparseDestroyMatDescr(descr);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCnnz_compress PASS\n");
    return 0;
}
