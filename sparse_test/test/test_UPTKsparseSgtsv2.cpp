/*
 * Auto-generated smoke test for UPTKsparseSgtsv2 (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgtsv2 setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgtsv2(sparse_handle, 0, 0, (const float*)dev_scratch, (const float*)dev_scratch, (const float*)dev_scratch, (float*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseSgtsv2 -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSgtsv2 PASS\n");
    return 0;
}
