/*
 * Auto-generated smoke test for UPTKsparseZgemvi (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgemvi setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgemvi(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, 0, 0, (const cuDoubleComplex*)dev_scratch, (const int*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseZgemvi -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseZgemvi PASS\n");
    return 0;
}
