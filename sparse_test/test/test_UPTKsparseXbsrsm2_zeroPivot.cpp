/*
 * Auto-generated smoke test for UPTKsparseXbsrsm2_zeroPivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXbsrsm2_zeroPivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXbsrsm2_zeroPivot(sparse_handle, (bsrsm2Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseXbsrsm2_zeroPivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXbsrsm2_zeroPivot PASS\n");
    return 0;
}
