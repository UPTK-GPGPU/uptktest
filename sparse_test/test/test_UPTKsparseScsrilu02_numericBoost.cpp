/*
 * Auto-generated smoke test for UPTKsparseScsrilu02_numericBoost (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScsrilu02_numericBoost setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseScsrilu02_numericBoost(sparse_handle, (csrilu02Info_t)0, 0, (double*)dev_scratch, (float*)dev_scratch);

    printf("UPTKsparseScsrilu02_numericBoost -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseScsrilu02_numericBoost PASS\n");
    return 0;
}
