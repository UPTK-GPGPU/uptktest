/*
 * Auto-generated smoke test for UPTKsparseSetPointerMode (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSetPointerMode setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSetPointerMode(sparse_handle, mode);

    printf("UPTKsparseSetPointerMode -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSetPointerMode PASS\n");
    return 0;
}
