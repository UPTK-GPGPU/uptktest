/*
 * Auto-generated smoke test for UPTKsparseCreateIdentityPermutation (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateIdentityPermutation setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateIdentityPermutation(sparse_handle, 0, (int*)dev_scratch);

    printf("UPTKsparseCreateIdentityPermutation -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCreateIdentityPermutation PASS\n");
    return 0;
}
