/*
 * Auto-generated smoke test for UPTKsparseSpVV (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpVV setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpVV(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, vecX, vecY, (void*)nullptr, (UPTKDataType_t)UPTK_R_32F, (void*)nullptr);

    printf("UPTKsparseSpVV -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpVV PASS\n");
    return 0;
}
