/*
 * Auto-generated smoke test for UPTKsparseSpMatSetValues (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatSetValues setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatSetValues(spMatDescr, (void*)nullptr);

    printf("UPTKsparseSpMatSetValues -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMatSetValues PASS\n");
    return 0;
}
