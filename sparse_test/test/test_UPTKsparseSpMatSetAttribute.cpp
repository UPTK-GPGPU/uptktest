/*
 * Auto-generated smoke test for UPTKsparseSpMatSetAttribute (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMatSetAttribute setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMatSetAttribute(spMatDescr, (UPTKsparseSpMatAttribute_t)0, (void*)nullptr, (size_t)0);

    printf("UPTKsparseSpMatSetAttribute -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpMatSetAttribute PASS\n");
    return 0;
}
