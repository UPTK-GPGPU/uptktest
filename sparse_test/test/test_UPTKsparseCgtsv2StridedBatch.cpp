/*
 * Auto-generated smoke test for UPTKsparseCgtsv2StridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgtsv2StridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgtsv2StridedBatch(sparse_handle, 0, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, 0, 0, (void*)nullptr);

    printf("UPTKsparseCgtsv2StridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgtsv2StridedBatch PASS\n");
    return 0;
}
