/*
 * Auto-generated smoke test for UPTKsparseDestroyBsrilu02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyBsrilu02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyBsrilu02Info((bsrilu02Info_t)0);

    printf("UPTKsparseDestroyBsrilu02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyBsrilu02Info PASS\n");
    return 0;
}
