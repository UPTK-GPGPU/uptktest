/*
 * Auto-generated smoke test for UPTKsparseSpMMOp (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMMOp setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMMOp(plan, (void*)nullptr);

    printf("UPTKsparseSpMMOp -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpMMOp PASS\n");
    return 0;
}
