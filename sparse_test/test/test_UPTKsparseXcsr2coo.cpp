/*
 * Auto-generated smoke test for UPTKsparseXcsr2coo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcsr2coo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcsr2coo(sparse_handle, (const int*)dev_scratch, 0, 0, (int*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO);

    printf("UPTKsparseXcsr2coo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXcsr2coo PASS\n");
    return 0;
}
