/*
 * Auto-generated smoke test for UPTKsparseRot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseRot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseRot(sparse_handle, (const void*)dev_scratch, (const void*)dev_scratch, vecX, vecY);

    printf("UPTKsparseRot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseRot PASS\n");
    return 0;
}
