/*
 * Auto-generated smoke test for UPTKsparseSgemvi (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgemvi setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgemvi(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, (const float*)dev_scratch, (const float*)dev_scratch, 0, 0, (const float*)dev_scratch, (const int*)dev_scratch, (const float*)dev_scratch, (float*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseSgemvi -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSgemvi PASS\n");
    return 0;
}
