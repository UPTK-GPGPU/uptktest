/*
 * Auto-generated smoke test for UPTKsparseDgtsv2StridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgtsv2StridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgtsv2StridedBatch(sparse_handle, 0, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, (double*)dev_scratch, 0, 0, (void*)nullptr);

    printf("UPTKsparseDgtsv2StridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDgtsv2StridedBatch PASS\n");
    return 0;
}
