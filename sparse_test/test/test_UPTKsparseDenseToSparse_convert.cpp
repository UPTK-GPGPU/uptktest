/*
 * Auto-generated smoke test for UPTKsparseDenseToSparse_convert (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDenseToSparse_convert setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDenseToSparse_convert(sparse_handle, matA, matB, (UPTKsparseDenseToSparseAlg_t)0, (void*)nullptr);

    printf("UPTKsparseDenseToSparse_convert -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDenseToSparse_convert PASS\n");
    return 0;
}
