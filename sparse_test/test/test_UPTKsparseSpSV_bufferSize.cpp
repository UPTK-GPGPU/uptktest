/*
 * Auto-generated smoke test for UPTKsparseSpSV_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSV_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSV_bufferSize(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, vecX, vecY, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpSVAlg_t)0, spsvDescr, (size_t*)dev_scratch);

    printf("UPTKsparseSpSV_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSV_bufferSize PASS\n");
    return 0;
}
