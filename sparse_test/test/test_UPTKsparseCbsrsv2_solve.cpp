/*
 * Auto-generated smoke test for UPTKsparseCbsrsv2_solve (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrsv2_solve setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrsv2_solve UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrsv2_solve SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsrsv2_solve SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCbsrsv2_solve(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, (const cuComplex*)dev_scratch, descrA, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, (bsrsv2Info_t)0, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, (UPTKsparseSolvePolicy_t)UPTKSPARSE_SOLVE_POLICY_NO_LEVEL, (void*)nullptr);

    printf("UPTKsparseCbsrsv2_solve -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCbsrsv2_solve PASS\n");
    return 0;
}
