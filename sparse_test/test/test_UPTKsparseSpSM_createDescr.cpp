/*
 * Auto-generated smoke test for UPTKsparseSpSM_createDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSM_createDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSM_createDescr((UPTKsparseSpSMDescr_t*)dev_scratch);

    printf("UPTKsparseSpSM_createDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSM_createDescr PASS\n");
    return 0;
}
