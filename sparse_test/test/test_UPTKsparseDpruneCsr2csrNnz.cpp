/*
 * Auto-generated smoke test for UPTKsparseDpruneCsr2csrNnz (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCreateMatDescr(&descrC);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz UPTKsparseCreateMatDescr(descrC) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrC, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz SetMatIndexBase(descrC)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDpruneCsr2csrNnz SetMatType(descrC)\n");
        UPTKsparseDestroyMatDescr(descrC); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseDpruneCsr2csrNnz(sparse_handle, 0, 0, 0, descrA, (const double*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, (const double*)dev_scratch, descrC, (int*)dev_scratch, (int*)dev_scratch, (void*)nullptr);

    printf("UPTKsparseDpruneCsr2csrNnz -> %d\n", (int)err);
    ...
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDpruneCsr2csrNnz PASS\n");
    return 0;
}
