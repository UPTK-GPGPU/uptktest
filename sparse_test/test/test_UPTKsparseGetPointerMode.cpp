/*
 * Auto-generated smoke test for UPTKsparseGetPointerMode (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseGetPointerMode setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseGetPointerMode(sparse_handle, (UPTKsparsePointerMode_t*)dev_scratch);

    printf("UPTKsparseGetPointerMode -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseGetPointerMode PASS\n");
    return 0;
}
