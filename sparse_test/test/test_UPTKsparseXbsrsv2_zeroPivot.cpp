/*
 * Auto-generated smoke test for UPTKsparseXbsrsv2_zeroPivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXbsrsv2_zeroPivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXbsrsv2_zeroPivot(sparse_handle, (bsrsv2Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseXbsrsv2_zeroPivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXbsrsv2_zeroPivot PASS\n");
    return 0;
}
