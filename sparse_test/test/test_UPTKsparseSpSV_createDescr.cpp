/*
 * Auto-generated smoke test for UPTKsparseSpSV_createDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSV_createDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSV_createDescr((UPTKsparseSpSVDescr_t*)dev_scratch);

    printf("UPTKsparseSpSV_createDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSV_createDescr PASS\n");
    return 0;
}
