/*
 * Auto-generated smoke test for UPTKsparseDestroyDnMat (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyDnMat setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyDnMat(dnMatDescr);

    printf("UPTKsparseDestroyDnMat -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDestroyDnMat PASS\n");
    return 0;
}
