/*
 * Auto-generated smoke test for UPTKsparseDestroyDnVec (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyDnVec setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyDnVec(dnVecDescr);

    printf("UPTKsparseDestroyDnVec -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyDnVec PASS\n");
    return 0;
}
