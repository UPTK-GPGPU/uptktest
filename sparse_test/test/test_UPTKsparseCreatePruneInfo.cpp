/*
 * Auto-generated smoke test for UPTKsparseCreatePruneInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreatePruneInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreatePruneInfo((pruneInfo_t*)dev_scratch);

    printf("UPTKsparseCreatePruneInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreatePruneInfo PASS\n");
    return 0;
}
