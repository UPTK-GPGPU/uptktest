/*
 * Auto-generated smoke test for UPTKsparseDestroyCsru2csrInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyCsru2csrInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyCsru2csrInfo((csru2csrInfo_t)0);

    printf("UPTKsparseDestroyCsru2csrInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyCsru2csrInfo PASS\n");
    return 0;
}
