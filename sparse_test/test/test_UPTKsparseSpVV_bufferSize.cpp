/*
 * Auto-generated smoke test for UPTKsparseSpVV_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpVV_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpVV_bufferSize(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, vecX, vecY, (const void*)dev_scratch, (UPTKDataType_t)UPTK_R_32F, (size_t*)dev_scratch);

    printf("UPTKsparseSpVV_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpVV_bufferSize PASS\n");
    return 0;
}
