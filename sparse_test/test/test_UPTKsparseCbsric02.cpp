/*
 * Auto-generated smoke test for UPTKsparseCbsric02 (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsric02 setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsric02 UPTKsparseCreateMatDescr(descrA) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrA, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsric02 SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCbsric02 SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseCbsric02(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, 0, 0, descrA, (cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, (bsric02Info_t)0, (UPTKsparseSolvePolicy_t)UPTKSPARSE_SOLVE_POLICY_NO_LEVEL, (void*)nullptr);

    printf("UPTKsparseCbsric02 -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCbsric02 PASS\n");
    return 0;
}
