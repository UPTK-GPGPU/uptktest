/*
 * Auto-generated smoke test for UPTKsparseCooSetPointers (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCooSetPointers setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCooSetPointers(spMatDescr, (void*)nullptr, (void*)nullptr, (void*)nullptr);

    printf("UPTKsparseCooSetPointers -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCooSetPointers PASS\n");
    return 0;
}
