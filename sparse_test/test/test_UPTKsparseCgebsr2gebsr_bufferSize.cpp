/*
 * Auto-generated smoke test for UPTKsparseCgebsr2gebsr_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgebsr2gebsr_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgebsr2gebsr_bufferSize UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgebsr2gebsr_bufferSize SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgebsr2gebsr_bufferSize SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCgebsr2gebsr_bufferSize(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, 0, 0, 0, descrA, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, 0, 0, (int*)dev_scratch);

    printf("UPTKsparseCgebsr2gebsr_bufferSize -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCgebsr2gebsr_bufferSize PASS\n");
    return 0;
}
