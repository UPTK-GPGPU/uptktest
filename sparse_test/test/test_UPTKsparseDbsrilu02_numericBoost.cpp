/*
 * Auto-generated smoke test for UPTKsparseDbsrilu02_numericBoost (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDbsrilu02_numericBoost setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDbsrilu02_numericBoost(sparse_handle, (bsrilu02Info_t)0, 0, (double*)dev_scratch, (double*)dev_scratch);

    printf("UPTKsparseDbsrilu02_numericBoost -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDbsrilu02_numericBoost PASS\n");
    return 0;
}
