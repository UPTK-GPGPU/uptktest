/*
 * Auto-generated smoke test for UPTKsparseZcsrilu02_numericBoost (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZcsrilu02_numericBoost setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZcsrilu02_numericBoost(sparse_handle, (csrilu02Info_t)0, 0, (double*)dev_scratch, (cuDoubleComplex*)dev_scratch);

    printf("UPTKsparseZcsrilu02_numericBoost -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseZcsrilu02_numericBoost PASS\n");
    return 0;
}
