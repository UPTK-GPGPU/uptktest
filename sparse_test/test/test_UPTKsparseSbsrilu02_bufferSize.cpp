/*
 * Auto-generated smoke test for UPTKsparseSbsrilu02_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSbsrilu02_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSbsrilu02_bufferSize UPTKsparseCreateMatDescr(descrA) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrA, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSbsrilu02_bufferSize SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSbsrilu02_bufferSize SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseSbsrilu02_bufferSize(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, 0, 0, descrA, (float*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, (bsrilu02Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseSbsrilu02_bufferSize -> %d\n", (int)err);



    if (descrA) UPTKsparseDestroyMatDescr(descrA);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSbsrilu02_bufferSize PASS\n");
    return 0;
}
