/*
 * Auto-generated smoke test for UPTKsparseCreateDnVec (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateDnVec setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateDnVec((UPTKsparseDnVecDescr_t*)dev_scratch, (int64_t)0, (void*)nullptr, (UPTKDataType_t)UPTK_R_32F);

    printf("UPTKsparseCreateDnVec -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCreateDnVec PASS\n");
    return 0;
}
