/*
 * Auto-generated smoke test for UPTKsparseCreateColorInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateColorInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateColorInfo((UPTKsparseColorInfo_t*)dev_scratch);

    printf("UPTKsparseCreateColorInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateColorInfo PASS\n");
    return 0;
}
