/*
 * Auto-generated smoke test for UPTKsparseScatter (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseScatter setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseScatter(sparse_handle, vecX, vecY);

    printf("UPTKsparseScatter -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseScatter PASS\n");
    return 0;
}
