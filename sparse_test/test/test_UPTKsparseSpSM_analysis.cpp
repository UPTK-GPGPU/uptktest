/*
 * Auto-generated smoke test for UPTKsparseSpSM_analysis (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSM_analysis setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSM_analysis(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, matB, matC, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpSMAlg_t)0, spsmDescr, (void*)nullptr);

    printf("UPTKsparseSpSM_analysis -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSM_analysis PASS\n");
    return 0;
}
