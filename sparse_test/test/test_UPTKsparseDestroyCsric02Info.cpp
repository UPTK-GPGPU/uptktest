/*
 * Auto-generated smoke test for UPTKsparseDestroyCsric02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyCsric02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyCsric02Info((csric02Info_t)0);

    printf("UPTKsparseDestroyCsric02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDestroyCsric02Info PASS\n");
    return 0;
}
