/*
 * Auto-generated smoke test for UPTKsparseSpruneDense2csrByPercentage (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage UPTKsparseCreateMatDescr(descrC) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrC, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage SetMatIndexBase(descrC)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage SetMatType(descrC)\n");
        UPTKsparseDestroyMatDescr(descrC); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseSpruneDense2csrByPercentage(sparse_handle, 0, 0, (const float*)dev_scratch, 0, 0.f, descrC, (float*)dev_scratch, (const int*)dev_scratch, (int*)dev_scratch, (pruneInfo_t)(uintptr_t)0, (void*)nullptr);

    printf("UPTKsparseSpruneDense2csrByPercentage -> %d\n", (int)err);



    if (descrC) UPTKsparseDestroyMatDescr(descrC);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpruneDense2csrByPercentage PASS\n");
    return 0;
}
