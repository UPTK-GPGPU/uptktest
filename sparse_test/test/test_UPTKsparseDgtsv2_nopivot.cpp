/*
 * Auto-generated smoke test for UPTKsparseDgtsv2_nopivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgtsv2_nopivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgtsv2_nopivot(sparse_handle, 0, 0, (const double*)dev_scratch, (const double*)dev_scratch, (const double*)dev_scratch, (double*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseDgtsv2_nopivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDgtsv2_nopivot PASS\n");
    return 0;
}
