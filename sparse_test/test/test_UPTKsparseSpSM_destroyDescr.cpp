/*
 * Auto-generated smoke test for UPTKsparseSpSM_destroyDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKsparseSpSMDescr_t descr{};

    UPTKsparseStatus_t err;

    err = sparse_setup_core(&sparse_handle, &dev_scratch, &stream_id, 1);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpSM_destroyDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpSM_destroyDescr(descr);

    printf("UPTKsparseSpSM_destroyDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpSM_destroyDescr PASS\n");
    return 0;
}
