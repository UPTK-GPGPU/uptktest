/*
 * Auto-generated smoke test for UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt UPTKsparseCreateMatDescr(descrC) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrC, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt SetMatIndexBase(descrC)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt SetMatType(descrC)\n");
        UPTKsparseDestroyMatDescr(descrC); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt(sparse_handle, 0, 0, (const float*)dev_scratch, 0, 0.f, descrC, (const float*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, (pruneInfo_t)(uintptr_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt -> %d\n", (int)err);



    if (descrC) UPTKsparseDestroyMatDescr(descrC);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpruneDense2csrByPercentage_bufferSizeExt PASS\n");
    return 0;
}
