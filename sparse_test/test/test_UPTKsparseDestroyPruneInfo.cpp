/*
 * Auto-generated smoke test for UPTKsparseDestroyPruneInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyPruneInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyPruneInfo((pruneInfo_t)(uintptr_t)0);

    printf("UPTKsparseDestroyPruneInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyPruneInfo PASS\n");
    return 0;
}
