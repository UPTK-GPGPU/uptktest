/*
 * Auto-generated smoke test for UPTKsparseGetProperty (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseGetProperty setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseGetProperty((libraryPropertyType)0, (int*)dev_scratch);

    printf("UPTKsparseGetProperty -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseGetProperty PASS\n");
    return 0;
}
