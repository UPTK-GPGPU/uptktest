/*
 * Auto-generated smoke test for UPTKsparseXcsrilu02_zeroPivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcsrilu02_zeroPivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcsrilu02_zeroPivot(sparse_handle, (csrilu02Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseXcsrilu02_zeroPivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseXcsrilu02_zeroPivot PASS\n");
    return 0;
}
