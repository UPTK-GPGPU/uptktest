/*
 * Auto-generated smoke test for UPTKsparseCreate (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    err = sparse_setup_core(&sparse_handle, &dev_scratch, &stream_id, 0);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreate setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreate(&sparse_handle);

    printf("UPTKsparseCreate -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreate PASS\n");
    return 0;
}
