/*
 * Auto-generated smoke test for UPTKsparseCgebsr2gebsc_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgebsr2gebsc_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgebsr2gebsc_bufferSize(sparse_handle, 0, 0, 0, (const cuComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, (int*)dev_scratch);

    printf("UPTKsparseCgebsr2gebsc_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgebsr2gebsc_bufferSize PASS\n");
    return 0;
}
