/*
 * Auto-generated smoke test for UPTKsparseDestroySpVec (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroySpVec setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroySpVec(spVecDescr);

    printf("UPTKsparseDestroySpVec -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDestroySpVec PASS\n");
    return 0;
}
