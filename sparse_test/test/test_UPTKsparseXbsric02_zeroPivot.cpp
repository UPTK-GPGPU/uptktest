/*
 * Auto-generated smoke test for UPTKsparseXbsric02_zeroPivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXbsric02_zeroPivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXbsric02_zeroPivot(sparse_handle, (bsric02Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseXbsric02_zeroPivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseXbsric02_zeroPivot PASS\n");
    return 0;
}
