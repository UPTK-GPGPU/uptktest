/*
 * Auto-generated smoke test for UPTKsparseDnVecGet (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKsparseDnVecDescr_t dnVecDescr{};

    UPTKsparseStatus_t err;

    err = sparse_setup_core(&sparse_handle, &dev_scratch, &stream_id, 1);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnVecGet setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnVecGet(dnVecDescr, (int64_t*)dev_scratch, (void**)nullptr, (UPTKDataType*)dev_scratch);

    printf("UPTKsparseDnVecGet -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnVecGet PASS\n");
    return 0;
}
