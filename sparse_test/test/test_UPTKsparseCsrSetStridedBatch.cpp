/*
 * Auto-generated smoke test for UPTKsparseCsrSetStridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCsrSetStridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCsrSetStridedBatch(spMatDescr, 0, (int64_t)0, (int64_t)0);

    printf("UPTKsparseCsrSetStridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCsrSetStridedBatch PASS\n");
    return 0;
}
