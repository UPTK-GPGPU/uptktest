/*
 * Auto-generated smoke test for UPTKsparseSDDMM_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSDDMM_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSDDMM_bufferSize(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, matB, (const void*)dev_scratch, matC, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSDDMMAlg_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseSDDMM_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSDDMM_bufferSize PASS\n");
    return 0;
}
