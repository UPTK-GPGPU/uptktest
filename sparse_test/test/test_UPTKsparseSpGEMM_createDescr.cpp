/*
 * Auto-generated smoke test for UPTKsparseSpGEMM_createDescr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpGEMM_createDescr setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpGEMM_createDescr((UPTKsparseSpGEMMDescr_t*)dev_scratch);

    printf("UPTKsparseSpGEMM_createDescr -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpGEMM_createDescr PASS\n");
    return 0;
}
