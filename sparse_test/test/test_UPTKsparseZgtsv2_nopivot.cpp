/*
 * Auto-generated smoke test for UPTKsparseZgtsv2_nopivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgtsv2_nopivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgtsv2_nopivot(sparse_handle, 0, 0, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseZgtsv2_nopivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseZgtsv2_nopivot PASS\n");
    return 0;
}
