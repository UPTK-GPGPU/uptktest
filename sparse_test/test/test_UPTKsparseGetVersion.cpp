/*
 * Auto-generated smoke test for UPTKsparseGetVersion (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseGetVersion setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseGetVersion(sparse_handle, (int*)dev_scratch);

    printf("UPTKsparseGetVersion -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseGetVersion PASS\n");
    return 0;
}
