/*
 * Auto-generated smoke test for UPTKsparseDnnz_compress (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnnz_compress setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnnz_compress UPTKsparseCreateMatDescr(descr) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descr, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnnz_compress SetMatIndexBase(descr)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnnz_compress SetMatType(descr)\n");
        UPTKsparseDestroyMatDescr(descr); sparse_teardown_core(sparse_handle, dev_scratch, stream_id); return 0;
    }

    err = UPTKsparseDnnz_compress(sparse_handle, 0, descr, (const double*)dev_scratch, (const int*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, 0.);

    printf("UPTKsparseDnnz_compress -> %d\n", (int)err);



    if (descr) UPTKsparseDestroyMatDescr(descr);
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnnz_compress PASS\n");
    return 0;
}
