/*
 * Auto-generated smoke test for UPTKsparseSpMM (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMM setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMM(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, matB, (const void*)dev_scratch, matC, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpMMAlg_t)0, (void*)nullptr);

    printf("UPTKsparseSpMM -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSpMM PASS\n");
    return 0;
}
