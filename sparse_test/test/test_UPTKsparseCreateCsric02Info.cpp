/*
 * Auto-generated smoke test for UPTKsparseCreateCsric02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateCsric02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateCsric02Info((csric02Info_t*)dev_scratch);

    printf("UPTKsparseCreateCsric02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateCsric02Info PASS\n");
    return 0;
}
