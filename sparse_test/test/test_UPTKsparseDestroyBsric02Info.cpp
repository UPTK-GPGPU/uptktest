/*
 * Auto-generated smoke test for UPTKsparseDestroyBsric02Info (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroyBsric02Info setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroyBsric02Info((bsric02Info_t)0);

    printf("UPTKsparseDestroyBsric02Info -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDestroyBsric02Info PASS\n");
    return 0;
}
