/*
 * Auto-generated smoke test for UPTKsparseZgebsr2gebsr (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr UPTKsparseCreateMatDescr(descrA) failed\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr SetMatIndexBase(descrA)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr SetMatType(descrA)\n");
        UPTKsparseDestroyMatDescr(descrA); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseCreateMatDescr(&descrC);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr UPTKsparseCreateMatDescr(descrC) failed\n");
        sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
        return 0;
    }
    err = UPTKsparseSetMatIndexBase(descrC, UPTKSPARSE_INDEX_BASE_ZERO);
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr SetMatIndexBase(descrC)\n");
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgebsr2gebsr SetMatType(descrC)\n");
        UPTKsparseDestroyMatDescr(descrC); sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1); return 0;
    }

    err = UPTKsparseZgebsr2gebsr(sparse_handle, (UPTKsparseDirection_t)UPTKSPARSE_DIRECTION_ROW, 0, 0, 0, descrA, (const cuDoubleComplex*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, descrC, (cuDoubleComplex*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, 0, 0, (void*)nullptr);

    printf("UPTKsparseZgebsr2gebsr -> %d\n", (int)err);
    ...
    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseZgebsr2gebsr PASS\n");
    return 0;
}
