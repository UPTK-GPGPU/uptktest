/*
 * Auto-generated smoke test for UPTKsparseGather (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseGather setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseGather(sparse_handle, vecY, vecX);

    printf("UPTKsparseGather -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseGather PASS\n");
    return 0;
}
