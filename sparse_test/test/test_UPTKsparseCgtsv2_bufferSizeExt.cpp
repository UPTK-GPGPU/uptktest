/*
 * Auto-generated smoke test for UPTKsparseCgtsv2_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgtsv2_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgtsv2_bufferSizeExt(sparse_handle, 0, 0, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, 0, (size_t*)dev_scratch);

    printf("UPTKsparseCgtsv2_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgtsv2_bufferSizeExt PASS\n");
    return 0;
}
