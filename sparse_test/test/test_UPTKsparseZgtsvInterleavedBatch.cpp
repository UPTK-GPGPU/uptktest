/*
 * Auto-generated smoke test for UPTKsparseZgtsvInterleavedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgtsvInterleavedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgtsvInterleavedBatch(sparse_handle, 0, 0, (cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, (cuDoubleComplex*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseZgtsvInterleavedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseZgtsvInterleavedBatch PASS\n");
    return 0;
}
