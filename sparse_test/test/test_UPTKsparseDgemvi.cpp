/*
 * Auto-generated smoke test for UPTKsparseDgemvi (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDgemvi setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDgemvi(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, 0, 0, (const double*)dev_scratch, (const double*)dev_scratch, 0, 0, (const double*)dev_scratch, (const int*)dev_scratch, (const double*)dev_scratch, (double*)dev_scratch, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseDgemvi -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDgemvi PASS\n");
    return 0;
}
