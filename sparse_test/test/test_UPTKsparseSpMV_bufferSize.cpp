/*
 * Auto-generated smoke test for UPTKsparseSpMV_bufferSize (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSpMV_bufferSize setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSpMV_bufferSize(sparse_handle, (UPTKsparseOperation_t)UPTKSPARSE_OPERATION_NON_TRANSPOSE, (const void*)dev_scratch, matA, vecX, (const void*)dev_scratch, vecY, (UPTKDataType_t)UPTK_R_32F, (UPTKsparseSpMVAlg_t)0, (size_t*)dev_scratch);

    printf("UPTKsparseSpMV_bufferSize -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseSpMV_bufferSize PASS\n");
    return 0;
}
