/*
 * Auto-generated smoke test for UPTKsparseSgebsr2gebsc (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseSgebsr2gebsc setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseSgebsr2gebsc(sparse_handle, 0, 0, 0, (const float*)dev_scratch, (const int*)dev_scratch, (const int*)dev_scratch, 0, 0, (float*)dev_scratch, (int*)dev_scratch, (int*)dev_scratch, (UPTKsparseAction_t)0, (UPTKsparseIndexBase_t)UPTKSPARSE_INDEX_BASE_ZERO, (void*)nullptr);

    printf("UPTKsparseSgebsr2gebsc -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseSgebsr2gebsc PASS\n");
    return 0;
}
