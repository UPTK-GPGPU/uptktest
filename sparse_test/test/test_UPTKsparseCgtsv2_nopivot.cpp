/*
 * Auto-generated smoke test for UPTKsparseCgtsv2_nopivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCgtsv2_nopivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCgtsv2_nopivot(sparse_handle, 0, 0, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (const cuComplex*)dev_scratch, (cuComplex*)dev_scratch, 0, (void*)nullptr);

    printf("UPTKsparseCgtsv2_nopivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCgtsv2_nopivot PASS\n");
    return 0;
}
