/*
 * Auto-generated smoke test for UPTKsparseCscSetPointers (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCscSetPointers setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCscSetPointers(spMatDescr, (void*)nullptr, (void*)nullptr, (void*)nullptr);

    printf("UPTKsparseCscSetPointers -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCscSetPointers PASS\n");
    return 0;
}
