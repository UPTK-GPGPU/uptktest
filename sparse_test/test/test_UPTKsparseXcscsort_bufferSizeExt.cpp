/*
 * Auto-generated smoke test for UPTKsparseXcscsort_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcscsort_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcscsort_bufferSizeExt(sparse_handle, 0, 0, 0, (const int*)dev_scratch, (const int*)dev_scratch, (size_t*)dev_scratch);

    printf("UPTKsparseXcscsort_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseXcscsort_bufferSizeExt PASS\n");
    return 0;
}
