/*
 * Auto-generated smoke test for UPTKsparseZgtsvInterleavedBatch_bufferSizeExt (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseZgtsvInterleavedBatch_bufferSizeExt setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseZgtsvInterleavedBatch_bufferSizeExt(sparse_handle, 0, 0, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, (const cuDoubleComplex*)dev_scratch, 0, (size_t*)dev_scratch);

    printf("UPTKsparseZgtsvInterleavedBatch_bufferSizeExt -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseZgtsvInterleavedBatch_bufferSizeExt PASS\n");
    return 0;
}
