/*
 * Auto-generated smoke test for UPTKsparseCooSetStridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCooSetStridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCooSetStridedBatch(spMatDescr, 0, (int64_t)0);

    printf("UPTKsparseCooSetStridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseCooSetStridedBatch PASS\n");
    return 0;
}
