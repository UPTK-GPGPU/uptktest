/*
 * Auto-generated smoke test for UPTKsparseDnMatSetStridedBatch (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDnMatSetStridedBatch setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDnMatSetStridedBatch(dnMatDescr, 0, (int64_t)0);

    printf("UPTKsparseDnMatSetStridedBatch -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseDnMatSetStridedBatch PASS\n");
    return 0;
}
