/*
 * Auto-generated smoke test for UPTKsparseCreateCsru2csrInfo (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseCreateCsru2csrInfo setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseCreateCsru2csrInfo((csru2csrInfo_t*)dev_scratch);

    printf("UPTKsparseCreateCsru2csrInfo -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseCreateCsru2csrInfo PASS\n");
    return 0;
}
