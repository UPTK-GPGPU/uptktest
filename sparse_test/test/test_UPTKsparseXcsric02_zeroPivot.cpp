/*
 * Auto-generated smoke test for UPTKsparseXcsric02_zeroPivot (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseXcsric02_zeroPivot setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseXcsric02_zeroPivot(sparse_handle, (csric02Info_t)0, (int*)dev_scratch);

    printf("UPTKsparseXcsric02_zeroPivot -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id);
    printf("test_UPTKsparseXcsric02_zeroPivot PASS\n");
    return 0;
}
