/*
 * Auto-generated smoke test for UPTKsparseDestroySpMat (sparse_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKSPARSE_STATUS_SUCCESS) {
        printf("test_skip: UPTKsparseDestroySpMat setup failed (%d)\n", (int)err);
        return 0;
    }


    err = UPTKsparseDestroySpMat(spMatDescr);

    printf("UPTKsparseDestroySpMat -> %d\n", (int)err);


    sparse_teardown_core(sparse_handle, dev_scratch, stream_id, 1);
    printf("test_UPTKsparseDestroySpMat PASS\n");
    return 0;
}
