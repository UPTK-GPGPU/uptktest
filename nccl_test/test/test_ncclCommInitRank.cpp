...
int main() {
    ...
    // 场景1: 基础 UPTKncclCommInitRank with nranks=1, rank=0
    {
        ...
        if (rank != 0) {
            printf("UPTKncclCommInitRank: expected rank=0, got %d\n", rank);
        ...
        if (count != 1) {
            printf("UPTKncclCommInitRank: expected count=1, got %d\n", count);
            UPTKncclCommDestroy(comm);
            return 1;
        }
    }
    ...
    printf("test_UPTKncclCommInitRank PASS\n");
    return 0;
}
