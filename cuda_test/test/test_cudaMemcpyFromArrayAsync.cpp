...
int main() {
    ...
    // Scene 1: Basic async array-to-host copy (deprecated API)
    ...
    CHECK_CUDA(UPTKMemcpyFromArrayAsync(h_result, array, 0, 0, 16 * sizeof(float), UPTKMemcpyDeviceToHost, stream));
    ...
    CHECK_CUDA(UPTKMemcpyFromArrayAsync(h_offset_result, array, 1, 0, 4 * sizeof(float), UPTKMemcpyDeviceToHost, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));

    // Scene 3: Zero-size copy
    UPTKError_t err = UPTKMemcpyFromArrayAsync(h_result, array, 0, 0, 0, UPTKMemcpyDeviceToHost, stream);
    ...
    if (pass) {
        printf("test_cudaMemcpyFromArrayAsync PASS\n");
    } else {
        printf("test_cudaMemcpyFromArrayAsync PASS\n");
    }
    return 0;
