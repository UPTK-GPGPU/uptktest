...
int main() {
    ...
    // Scene 1: UPTKMemcpy3DPeerAsync - skip if only 1 device
    if (deviceCount < 2) {
        printf("test_skip: UPTKMemcpy3DPeerAsync requires at least 2 devices, found %d\n", deviceCount);
        return 0;
    }

    // Scene 2: Basic UPTKMemcpy3DPeerAsync between device 0 and device 1
    ...
    CHECK_CUDA(UPTKMemcpy3DPeerAsync(&peerParams, stream));
    ...
    printf("test_cudaMemcpy3DPeerAsync PASS\n");
    return 0;
}
