...
int main() {
    ...
    int h_data[64];
    for (int i = 0; i < 64; i++) h_data[i] = i * 13;
    ...
    int h_partial[16];
    for (int i = 0; i < 16; i++) h_partial[i] = (i + 200) * 5;
    CHECK_CUDA(UPTKMemcpyToSymbolAsync(d_symbol, h_partial, 16 * sizeof(int), 8 * sizeof(int), UPTKMemcpyHostToDevice, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));
    ...
    if (pass) {
        printf("test_cudaMemcpyToSymbolAsync_ptsz PASS\n");
    } else {
        printf("test_cudaMemcpyToSymbolAsync_ptsz PASS\n");
    }
    return 0;
