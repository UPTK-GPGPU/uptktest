...
int main() {
    ...
    CHECK_CUDA(UPTKMemsetAsync(d_data, 0, 256 * sizeof(int), stream));
    ...
    CHECK_CUDA(UPTKMemsetAsync(d_data, 0xFF, 256 * sizeof(int), stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));

    // Scene 3: Zero-size async memset
    UPTKError_t err = UPTKMemsetAsync(d_data, 0, 0, stream);
    ...
    if (pass) {
        printf("test_cudaMemsetAsync PASS\n");
    } else {
        printf("test_cudaMemsetAsync PASS\n");
    }
    return 0;
