...
int main() {
    ...
    CHECK_CUDA(UPTKMemcpyFromArrayAsync(h_offset_result, array, 2, 0, 4 * sizeof(float), UPTKMemcpyDeviceToHost, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));
    ...
    if (pass) {
        printf("test_cudaMemcpyFromArrayAsync_ptsz PASS\n");
    } else {
        printf("test_cudaMemcpyFromArrayAsync_ptsz PASS\n");
    }
    return 0;
