...
int main() {
    ...
    // UPTKGraphicsResourceGetMappedPointer requires a mapped graphics resource.
    ...
    UPTKError_t err = UPTKGraphicsResourceGetMappedPointer(&devPtr, &size, badResource);
    ...
    // Scenario 2: NULL resource
    err = UPTKGraphicsResourceGetMappedPointer(&devPtr, &size, NULL);
    if (err != UPTKErrorInvalidResourceHandle && err != UPTKErrorNotSupported && err != UPTKErrorInvalidValue) {
        printf("FAIL: expected error for NULL resource, got %s\n", UPTKGetErrorString(err));
        return 1;
    }

    printf("test_cudaGraphicsResourceGetMappedPointer PASS\n");
    return 0;
}
