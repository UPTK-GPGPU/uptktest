...
int main() {
    ...
    // Scene 1: Basic UPTKMemPoolTrimTo
    ...
    void *d_ptr = NULL;
    CHECK_CUDA(UPTKMallocFromPoolAsync(&d_ptr, 1024 * 1024, memPool, stream));
    CHECK_CUDA(UPTKFreeAsync(d_ptr, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));

    // Trim pool to keep minimum bytes
    CHECK_CUDA(UPTKMemPoolTrimTo(memPool, 0));

    CHECK_CUDA(UPTKStreamDestroy(stream));
    CHECK_CUDA(UPTKMemPoolDestroy(memPool));

    printf("test_cudaMemPoolTrimTo PASS\n");
    return 0;
}
