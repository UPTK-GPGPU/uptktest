...
int main() {
    ...
    // Scene 1: cudaMemcpy3DPeer_ptds - skip if only 1 device
    if (deviceCount < 2) {
        printf("test_skip: cudaMemcpy3DPeer_ptds requires at least 2 devices, found %d\n", deviceCount);
        return 0;
    }

    // Scene 2: Basic cudaMemcpy3DPeer_ptds between device 0 and device 1
    ...
    printf("test_cudaMemcpy3DPeer_ptds PASS\n");
    return 0;
}
