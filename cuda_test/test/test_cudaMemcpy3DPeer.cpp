...
int main() {
    ...
    // Scene 1: UPTKMemcpy3DPeer - skip if only 1 device (peer copy requires multiple devices)
    if (deviceCount < 2) {
        printf("test_skip: UPTKMemcpy3DPeer requires at least 2 devices, found %d\n", deviceCount);
        return 0;
    }

    // Scene 2: Basic UPTKMemcpy3DPeer between device 0 and device 1
    ...
    CHECK_CUDA(UPTKMemcpy3DPeer(&peerParams));
    
    // Copy back to host from device 1
    memset(&params, 0, sizeof(params));
    params.srcPtr = d_dst;
    ...
    printf("test_cudaMemcpy3DPeer PASS\n");
    return 0;
}
