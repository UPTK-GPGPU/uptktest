...
int main() {
    ...
    UPTKError_t err = UPTKGraphMemsetNodeGetParams((UPTKGraphNode_t)0xDEADBEEF, &badParams);
    ...
    err = UPTKGraphMemsetNodeGetParams((UPTKGraphNode_t)0xDEADBEEF, NULL);
    if (err != UPTKErrorInvalidValue) {
        printf("FAIL: expected UPTKErrorInvalidValue for NULL params, got %s\n", UPTKGetErrorString(err));
        return 1;
    }

    printf("test_cudaGraphMemsetNodeGetParams PASS\n");
    return 0;
}
