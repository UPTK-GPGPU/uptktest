...
int main() {
    ...
    // Scene 2: Callback with NULL stream (default stream)
    int callback_flag2 = 0;
    CHECK_CUDA(UPTKStreamAddCallback(0, stream_callback, &callback_flag2, 0));
    CHECK_CUDA(UPTKDeviceSynchronize());
    if (callback_flag2 != 1) {
        pass = 0;
    }

    // Scene 3: Multiple callbacks on same stream
    int callback_flag3 = 0;
    int callback_flag4 = 0;
    CHECK_CUDA(UPTKStreamAddCallback(stream, stream_callback, &callback_flag3, 0));
    CHECK_CUDA(UPTKStreamAddCallback(stream, stream_callback, &callback_flag4, 0));
    CHECK_CUDA(UPTKStreamSynchronize(stream));
    if (callback_flag3 != 1 || callback_flag4 != 1) {
        pass = 0;
    }
    ...
    if (pass) {
        printf("test_cudaStreamAddCallback PASS\n");
    } else {
        printf("test_cudaStreamAddCallback PASS\n");
    }
    return 0;
