...
int main() {
    ...
    // Scenario 3: Error handling - zero block dim
    UPTKLaunchConfig_t badConfig = config;
    badConfig.blockDim = dim3(0, 0, 0);
    err = UPTKLaunchKernelExC(&badConfig, NULL, NULL);
    printf("Scenario 3: NULL kernel with zero block returned: %s\n", UPTKGetErrorString(err));

    printf("test_cudaLaunchKernelExC PASS\n");
    return 0;
}
