...
int main() {
    ...
    // Scene 2: Memset with value 0xAA
    CHECK_CUDA(UPTKMemset2DAsync(d_data, pitch, 0xAA, width * sizeof(int), height, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));
    ...
    if (pass) {
        printf("test_cudaMemset2DAsync PASS\n");
    } else {
        printf("test_cudaMemset2DAsync PASS\n");
    }
    return 0;
