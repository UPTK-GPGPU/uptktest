...
int main() {
    ...
    // Scene 1: Basic async symbol-to-host copy
    int h_data[64];
    for (int i = 0; i < 64; i++) h_data[i] = i * 3;
    ...
    int h_partial[32];
    CHECK_CUDA(UPTKMemcpyFromSymbolAsync(h_partial, d_symbol, 32 * sizeof(int), 16 * sizeof(int), UPTKMemcpyDeviceToHost, stream));
    CHECK_CUDA(UPTKStreamSynchronize(stream));
    for (int i = 0; i < 32; i++) {
    ...
    if (pass) {
        printf("test_cudaMemcpyFromSymbolAsync PASS\n");
    } else {
        printf("test_cudaMemcpyFromSymbolAsync PASS\n");
    }
    return 0;
