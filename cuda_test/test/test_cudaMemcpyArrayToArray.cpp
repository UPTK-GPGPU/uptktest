...
int main() {
    ...
    // Scene 1: Basic UPTKMemcpyArrayToArray
    ...
    CHECK_CUDA(UPTKMemcpyArrayToArray(dstArray, 0, 0, srcArray, 0, 0,
        width * height * sizeof(float), UPTKMemcpyDeviceToDevice));
    ...
    // Scene 2: UPTKMemcpyArrayToArray with offsets
    ...
    CHECK_CUDA(UPTKMemcpyArrayToArray(dstArray2, 8, 8, srcArray2, 4, 4,
        16 * 16 * sizeof(float), UPTKMemcpyDeviceToDevice));
    CHECK_CUDA(UPTKFreeArray(srcArray2));
    CHECK_CUDA(UPTKFreeArray(dstArray2));

    printf("test_cudaMemcpyArrayToArray PASS\n");
    return 0;
}
