...
int main() {
    ...
    // Scene 1: Basic UPTKMemcpy2DFromArrayAsync
    ...
    CHECK_CUDA(UPTKMemcpy2DFromArrayAsync(h_dst, width * sizeof(float), array, 0, 0,
    ...
    // Scene 2: UPTKMemcpy2DFromArrayAsync with default stream
    UPTKArray_t array2 = NULL;
    CHECK_CUDA(UPTKMallocArray(&array2, &channelDesc, 32, 32));
    float *h_dst2 = (float*)malloc(32 * 32 * sizeof(float));
    CHECK_CUDA(UPTKMemcpy2DFromArrayAsync(h_dst2, 32 * sizeof(float), array2, 0, 0,
        32 * sizeof(float), 32, UPTKMemcpyDeviceToHost, 0));
    CHECK_CUDA(UPTKDeviceSynchronize());
    CHECK_CUDA(UPTKFreeArray(array2));
    free(h_dst2);

    printf("test_cudaMemcpy2DFromArrayAsync PASS\n");
    return 0;
}
