...
#define SECTION(...) printf("%s", __VA_ARGS__);
...
inline float cudaSRGBMap(float fc) {
...
inline float cudaSRGBUnmap(float fc) {
  double c = static_cast<double>(fc);
...
inline float4 cudaSRGBMap(float4 fc) {
  fc.x = cudaSRGBMap(fc.x);
  fc.y = cudaSRGBMap(fc.y);
  fc.z = cudaSRGBMap(fc.z);
  // Alpha channel will keep unchanged
  return fc;
}

inline float4 cudaSRGBUnmap(float4 fc) {
  fc.x = cudaSRGBUnmap(fc.x);
  fc.y = cudaSRGBUnmap(fc.y);
  fc.z = cudaSRGBUnmap(fc.z);
  // Alpha channel will keep unchanged
  return fc;
}
...
template<typename T, UPTKTextureFilterMode fMode = UPTKFilterModePoint, bool sRGB = false>
bool cudaTextureSamplingVerify(T outputData, T expected) {
  bool testResult = false;
  if (fMode == UPTKFilterModePoint && !sRGB) {
    testResult = outputData == expected;
  } else {
    double mean = (cudaFabs(outputData) + cudaFabs(expected)) / 2;
    double diff = cudaFabs(outputData - expected);
...
template<UPTKTextureAddressMode addressMode>
void cudaTextureGetAddress(int &value, const int maxValue)
{
  switch(addressMode)
  {
...
// x, y and z must be returned by cudaTextureGetAddress()
template<typename T, UPTKTextureAddressMode addressMode, bool sRGB = false>
T cudaTextureGetValue(const T *data, const int x, const int width,
  ...
  if (sRGB && std::is_same<T, float4>::value) {
    result = cudaSRGBUnmap(result);
  }
  return result;
}

template<typename T, UPTKTextureAddressMode addressMode, UPTKTextureFilterMode filterMode, bool sRGB = false>
T getExpectedValue(const int width, float x, const T *data) {
  ...
  switch (filterMode) {
    case UPTKFilterModePoint: {
      ...
      cudaTextureGetAddress < addressMode > (i1, width);
      result = cudaTextureGetValue < T, addressMode, sRGB > (data, i1, width);
    }
      break;
    case UPTKFilterModeLinear: {
      ...
      float a = x - i1;
      cudaTextureGetAddress < addressMode > (i1, width);
      cudaTextureGetAddress < addressMode > (i2, width);

      T t1 = cudaTextureGetValue < T, addressMode, sRGB> (data, i1, width);
      T t2 = cudaTextureGetValue < T, addressMode, sRGB > (data, i2, width);
...
T getExpectedValue(const int width, const int height, float x, float y, const T *data) {
  ...
  switch (filterMode) {
    case UPTKFilterModePoint: {
      ...
      cudaTextureGetAddress < addressMode > (i1, width);
      cudaTextureGetAddress < addressMode > (j1, height);
      result = cudaTextureGetValue < T, addressMode, sRGB > (data, i1, width, j1, height);
    }
      break;
    case UPTKFilterModeLinear: {
      x -= 0.5;
      ...
      cudaTextureGetAddress < addressMode > (i2, width);
      cudaTextureGetAddress < addressMode > (j1, height);
      cudaTextureGetAddress < addressMode > (j2, height);

      T t11 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j1, height);
      T t21 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i2, width, j1, height);
      T t12 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j2, height);
      T t22 = cudaTextureGetValue < T, addressMode, sRGB
...
template<class T, UPTKTextureAddressMode addressMode, UPTKTextureFilterMode filterMode, bool sRGB = false>
T getExpectedValue(const int width, const int height, const int depth,
  ...
  switch (filterMode) {
    case UPTKFilterModePoint: {
      ...
      cudaTextureGetAddress < addressMode > (i1, width);
      cudaTextureGetAddress < addressMode > (j1, height);
      cudaTextureGetAddress < addressMode > (k1, depth);

      result = cudaTextureGetValue < T, addressMode, sRGB > (data, i1, width, j1, height, k1, depth);
    }
      break;
    case UPTKFilterModeLinear: {
      x -= 0.5;
      ...
      cudaTextureGetAddress < addressMode > (j2, height);
      cudaTextureGetAddress < addressMode > (k1, depth);
      cudaTextureGetAddress < addressMode > (k2, depth);

      T t111 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j1, height, k1, depth);
      T t211 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i2, width, j1, height, k1, depth);
      T t121 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j2, height, k1, depth);
      T t112 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j1, height, k2, depth);
      T t122 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i1, width, j2, height, k2, depth);
      T t212 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i2, width, j1, height, k2, depth);
      T t221 = cudaTextureGetValue < T, addressMode, sRGB
          > (data, i2, width, j2, height, k1, depth);
      T t222 = cudaTextureGetValue < T, addressMode, sRGB
