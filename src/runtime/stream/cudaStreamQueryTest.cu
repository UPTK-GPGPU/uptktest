...
__global__ void vector_square3(float* C_d, float* A_d, size_t n) {
...
TEST(cudaStream, cudaStreamQueryTest){
    ...
    ///hipLaunchKernelGGL((vector_square3), dim3(blocks), dim3(threadsPerBlock), 0, mystream, C_d, A_d,
    ///                   N);
    vector_square3<<<dim3(blocks), dim3(threadsPerBlock), 0, mystream>>>(C_d, A_d, N);
