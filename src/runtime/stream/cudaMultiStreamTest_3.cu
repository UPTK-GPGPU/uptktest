...
void MatrixCPUAddition_07(int* a, int *b, int*c) {
    for(int i=0;i<N;i++) 
    {  
        c[i] = (a[i] + b[i]) / 2;  
    }
...
__global__ void MatrixAddition_07(int* a, int *b, int*c) {
...
TEST(cudaStream, cudaMultiStreamTest_3){
    ...
    for(int i=0;i<STREAM_NUM;i++){
        if(i!=0){
            ...
            //hipLaunchKernelGGL(MatrixAddition_07, streamSize/WIDTH , WIDTH, 0 , mystream[i] , &A_d[streamSize*i], &B_d[streamSize*i], &C_d[streamSize*i]);
            MatrixAddition_07<<<streamSize/WIDTH , WIDTH, 0 , mystream[i]>>>(&A_d[streamSize*i], &B_d[streamSize*i], &C_d[streamSize*i]);
            ...
            ///hipLaunchKernelGGL(MatrixAddition_07, streamSize/WIDTH , WIDTH, 0 , 0, &A_d[streamSize*i], &B_d[streamSize*i], &C_d[streamSize*i]);
            MatrixAddition_07<<<streamSize/WIDTH , WIDTH>>>(&A_d[streamSize*i], &B_d[streamSize*i], &C_d[streamSize*i]);
    ...
    UPTKStreamSynchronize(NULL);
    MatrixCPUAddition_07(A_h,B_h,Result_h);
    // verify the results
    int errors = 0;
    double eps = 1.0E-6;
    for (int i = 0; i < N; i++) {
