
#include <stdio.h>
#include <thread>
#include <gtest/gtest.h>
#include <stddef.h>

#include <cuda.h>
#include <cuda_runtime_api.h>
#include <UPTK_runtime_api.h>
#include <UPTK_runtime.h>

#define N 1024

__global__ void vector_square(float* C_d, float* A_d, size_t n) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;

    if (i < n) C_d[i] = A_d[i] * A_d[i];
    
}

bool cbDone = false;
float *A_h, *C_h;
static void CUDART_CB Callback(UPTKStream_t stream, UPTKError_t status, void* userData) { 
    for (size_t i = 0; i < N; i++) {
        //if (C_h[i] != A_h[i] * A_h[i]) {
        //    printf("Data mismatch %zu", i);
        //}
        EXPECT_TRUE(C_h[i]==A_h[i] * A_h[i]) << "Data mismatch\n";
    }
    printf("test success!\n");
    cbDone = true;
}

TEST(cudaStream, cudaStreamAddCallbackTest){
    UPTKError_t ret = UPTKSuccess;
    float *A_d, *C_d;
    size_t Nbytes = N * sizeof(float);

    A_h = (float *)malloc(Nbytes);
    EXPECT_TRUE(A_h!=NULL);
    C_h = (float *)malloc(Nbytes);
    EXPECT_TRUE(C_h!=NULL);

    for (size_t i = 0; i < N; i++) {
        A_h[i] = 1.618f + i;
    }

    ret = UPTKMalloc(&A_d, Nbytes);
    EXPECT_EQ(ret, UPTKSuccess);
    ret = UPTKMalloc(&C_d, Nbytes);
    EXPECT_EQ(ret, UPTKSuccess);

    UPTKStream_t mystream;
    ret = UPTKStreamCreateWithFlags(&mystream, UPTKStreamNonBlocking);//cudaStreamNonBlocking使得非空流的阻塞行为消失,消除非空流对空流的阻塞行为
    EXPECT_EQ(ret, UPTKSuccess);
    ret = UPTKMemcpyAsync(A_d, A_h, Nbytes, UPTKMemcpyHostToDevice, mystream);
    EXPECT_EQ(ret, UPTKSuccess);

    const unsigned blocks = 512;
    const unsigned threadsPerBlock = 256;

    //hipLaunchKernelGGL(vector_square, dim3(blocks), dim3(threadsPerBlock), 0, mystream, C_d, A_d,N);
    vector_square<<<dim3(blocks), dim3(threadsPerBlock), 0, mystream>>> (C_d, A_d,N);

    ret = UPTKMemcpyAsync(C_h, C_d, Nbytes, UPTKMemcpyDeviceToHost, mystream);
    EXPECT_EQ(ret, UPTKSuccess);
    ret = UPTKStreamAddCallback(mystream, Callback, NULL, 0);
    EXPECT_EQ(ret, UPTKSuccess);

    while (!cbDone) std::this_thread::sleep_for(std::chrono::milliseconds(10));
    free(A_h);
    free(C_h);
    UPTKFree(A_d);
    UPTKFree(C_d);
    UPTKStreamDestroy(mystream);

}