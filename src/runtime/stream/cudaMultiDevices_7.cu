...
void MatrixCPUAddition_17(int* a, int *b, int*c) {
    for(int i=0;i<N;i++) 
    {  
        c[i] = (a[i] + b[i]) / 2;  
    }
...
__global__ void MatrixAddition_17(int* a, int *b, int*c) {
...
__global__ void MatrixAddition_17zero(int* a, int *b, int*c) {
...
TEST(cudaStream, cudaMultiDevices_7){
    ...
    UPTKMemcpyAsync(&B_d[0], &B_h[0] , streamBytes ,UPTKMemcpyHostToDevice,mystream[2]);
    ///hipLaunchKernelGGL(MatrixAddition_17, streamSize/WIDTH , WIDTH, 0 , mystream[0] , &A_d[0], &B_d[0], &C_d[0]);
    MatrixAddition_17<<<streamSize/WIDTH , WIDTH, 0 , mystream[0]>>> (&A_d[0], &B_d[0], &C_d[0]);
    ...
    //hipLaunchKernelGGL(MatrixAddition_17, streamSize/WIDTH , WIDTH, 0 , mystream[1] , &A_d[streamSize], &B_d[streamSize], &C_d[streamSize]);
    MatrixAddition_17<<<streamSize/WIDTH , WIDTH, 0 , mystream[1]>>> (&A_d[streamSize], &B_d[streamSize], &C_d[streamSize]);
    ...
    */
    ...
    MatrixCPUAddition_17(A_h,B_h,Result_h);
