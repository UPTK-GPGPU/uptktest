...
void MatrixCPUAddition_03(int* a, int *b, int*c) {
    for(int i=0;i<N;i++) 
    {  
        c[i] = (a[i] + b[i]) / 2;  
    }
...
__global__ void MatrixAddition_03(int* a, int *b, int*c) {
...
__global__ void MatrixAddition_zero3(int* a, int *b, int*c) {
...
TEST(cudaStream, cudaMultiStreamTest){
    ...
    MatrixCPUAddition_03(A_h,B_h,Result_h);
    ...
    ///hipLaunchKernelGGL(MatrixAddition_03, streamSize/WIDTH , WIDTH, 0 , 0, A_d, B_d, C_d);
    MatrixAddition_03<<<streamSize/WIDTH , WIDTH>>> (A_d, B_d, C_d);
    ...
    for(int j=1;j<1000;j++){
        for(int i=0;i<STREAM_NUM;i++){
        ///hipLaunchKernelGGL(MatrixAddition_zero3, streamSize/WIDTH , WIDTH, 0, mystream[i] , A_d, B_d, C_d);
        MatrixAddition_zero3<<<streamSize/WIDTH , WIDTH, 0, mystream[i]>>>(A_d, B_d, C_d);
        ...
        ///hipLaunchKernelGGL(MatrixAddition_03, streamSize/WIDTH , WIDTH, 0, mystream[i] , A_d, B_d, C_d);
        MatrixAddition_03<<<streamSize/WIDTH , WIDTH, 0, mystream[i]>>> (A_d, B_d, C_d);
