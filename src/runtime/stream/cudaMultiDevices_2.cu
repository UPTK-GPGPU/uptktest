...
void MatrixCPUAddition_11(int* a, int *b, int*c) {
    for(int i=0;i<N;i++) 
    {  
        c[i] = (a[i] + b[i]) / 2;  
    }
...
__global__ void MatrixAddition_11(int* a, int *b, int*c) {
...
__global__ void MatrixAddition_zero11(int* a, int *b, int*c) {
...
TEST(cudaStream, cudaMultiDevices_2){
    ...
    MatrixCPUAddition_11(A_h,B_h,Result_h);
    ...
    ///hipLaunchKernelGGL(MatrixAddition_11, streamSize/WIDTH , WIDTH, 0 , 0, A_d, B_d, C_d);
    MatrixAddition_11<<<streamSize/WIDTH , WIDTH>>> (A_d, B_d, C_d);
    ...
    for(int i=0;i<STREAM_NUM;i++){
        for(int j=1;j<repeat;j++){
            ret= UPTKSetDevice(STREAM_NUM % 8);
            EXPECT_EQ(ret, UPTKSuccess);
            ///hipLaunchKernelGGL(MatrixAddition_zero11, streamSize/WIDTH , WIDTH, 0, mystream[i] , A_d, B_d, C_d);
            MatrixAddition_zero11<<<streamSize/WIDTH , WIDTH, 0, mystream[i]>>> (A_d, B_d, C_d);
            ...
            ///hipLaunchKernelGGL(MatrixAddition_11, streamSize/WIDTH , WIDTH, 0, mystream[i] , A_d, B_d, C_d);
            MatrixAddition_11<<<streamSize/WIDTH , WIDTH, 0, mystream[i]>>>(A_d, B_d, C_d);
        ...
        if(j%10==0){
            for (int k = 0; k < N; k++) {
                if (std::abs(C_h[k] - Result_h[k]) > eps) {
                    errors++;
                    }
