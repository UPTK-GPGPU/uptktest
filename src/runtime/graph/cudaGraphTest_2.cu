/*
** test_file: hipGraphTest_2.cpp
** brief:     hipGraph SPACE_KERNEL = 0 (kernel operation)
*/
...
#define CUDACHECK(error)                                                                            \
    {                                                                                              \
        ...
        if ((localError != UPTKSuccess) && (localError != UPTK_ERROR_PEER_ACCESS_ALREADY_ENABLED)) {      \
...
#define TESTSIZE_OF_2_POWER_MAX     16
...
__global__ void add_2(float *d_x, float *d_y, float *d_z, size_t N) {
...
bool hipSerialKernelLaunch_2(float *inputVec_h, float *inputVec_2_h,
    ...
    for (size_t i = 0; i < kernelNum; i++) {
        //hipLaunchKernelGGL(add_2, gridSize, blockSize, 0, streamSerial , inputVec_d, inputVec_2_d, outputVec_d, inputSize);
        add_2<<<gridSize, blockSize, 0, streamSerial>>> (inputVec_d, inputVec_2_d, outputVec_d, inputSize);
    ...
    logtime2[0] = time1 - startTime;
    logtime2[1] = stopTime - startTime;
...
bool hipGraphNodeKernelLaunch_2(float *inputVec_h, float *inputVec_2_h,
                              float *inputVec_d, float *inputVec_2_d,
                              float *outputVec_d, 
                              float *result_h,
                              size_t inputSize, 
    ...
    stopTime = getRealTime();
    logtime2[7] = stopTime - startTime;

    startTime = getRealTime();
    CUDACHECK(UPTKGraphDestroy(graph));
    stopTime = getRealTime();
    logtime2[8] = stopTime - startTime;
...
bool hipGraphCaptureKernelLaunch_2(float *inputVec_h, float *inputVec_2_h,
                                 float *inputVec_d, float *inputVec_2_d,
            ...
            for (kernelNum = kernelNumStart; kernelNum <= kernelNumEnd; kernelNum *= 2) {
                ...
                for (gridSize = gridSizeStart; gridSize <= gridSizeEnd; gridSize += 63) {
                     ...
                     for (blockSize = blockSizeStart; blockSize <= blockSizeEnd; blockSize *= 2) {
                        ...
                        bool statusSerial = hipSerialKernelLaunch_2(inputVec_h, inputVec_2_h, inputVec_d, inputVec_2_d, outputVec_d,
                                                                    result_h, size, kernelNum, blockSize, gridSize);


                        bool statusNode = hipGraphNodeKernelLaunch_2(inputVec_h, inputVec_2_h, inputVec_d, inputVec_2_d, outputVec_d,
                                                                     result_h, size, kernelNum, blockSize, gridSize);

                        //bool status3 = hipGraphCaptureKernelLaunch_2(inputVec_h, inputVec_d, outputVec_d, result_d, size, maxBlocks);
                        ...
                        dispatch_totaltime = logtime2[2] + logtime2[3] + logtime2[4] + logtime2[5] + logtime2[7] + logtime2[8];
                        launch_totaltime = logtime2[2] + logtime2[3] + logtime2[4] + logtime2[6] + logtime2[7] + logtime2[8];
                        printf("%d %zu %zu %zu %zu %f %f %f %f %f %f %f %f %f %f %f\n",
                                loopnum, size, kernelNum, gridSize, blockSize,
                                logtime2[0], logtime2[1], logtime2[2], logtime2[3], logtime2[4], logtime2[5], logtime2[6], logtime2[7], logtime2[8],
...
TEST(cudaGraph,cudaGraphTest_2) {
    graphTest_2();
}
