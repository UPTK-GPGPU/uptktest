...
void *csr_mm(void *data) {
    ...
    ret = UPTKSetDevice(id);
    if (ret) {
        printf("set device error:%d, device id:%d\n", ret,id);
        return NULL;
    }
...
TEST(cudaKernel,cudaKerneltest6){
    //创建一个流队列，有两个流
    ...
    int * thread = (int *)malloc(sizeof(int) * deviceCnt);
    
    pthread_t tid[THREAD_NUM];
    for (int i = 0; i < deviceCnt; i++) {
        thread[i] = i;
        pthread_create(&tid[i], NULL, csr_mm, (void *) &thread[i]);
