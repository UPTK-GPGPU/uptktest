...
//        CreateDM(den_value_c,0.0f);
    ...
    //        printf("index:%d thread_id=%u, stream:%p\n",i,(unsigned int)pthread_self(),StreamData);
     //   }
        ...
        int number=(M * N);
        void* Args1[]={&d_C_value, &number};
        HIP_CHECK(UPTKLaunchKernel((const void *)set_C_to_1024, gridsize,blocksize, Args1,0,temp));
        ...
        void* Args2[]={&d_C_value, &number};
        HIP_CHECK(UPTKLaunchKernel((const void *)set_C_to_1, gridsize,blocksize, Args2,0,temp));
        ...
        void* Args3[]={&d_C_value,&d_B_value, &number};
        HIP_CHECK(UPTKLaunchKernel((const void *)set_C_copy_to_B, gridsize,blocksize, Args3,0,temp));
        ...
        void* Args4[]={&d_B_value, &number};
        HIP_CHECK(UPTKLaunchKernel((const void *)set_B_to_124, gridsize,blocksize, Args4,0,temp));
        UPTKStreamSynchronize(temp);
        //usleep(500 * 1);
        #if 0 
        ...
        void* Args5[]={&d_B_value, &number};
        HIP_CHECK(UPTKLaunchKernel((const void *)set_B_to_256, gridsize,blocksize, Args5,0,temp));
        UPTKStreamSynchronize(temp);
        //usleep(500 * 1);
        #if 0 
...
TEST(cudaKernel,cudaKerneltest3){
    //创建一个流队列，有两个流
