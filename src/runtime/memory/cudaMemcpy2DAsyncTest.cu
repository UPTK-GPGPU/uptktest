...
TEST(cudaMemory,cudaMemcpy2DAsyncTest){
...
//#ifndef __TEST_HIPHSA__
    ...
    ret = UPTKMemcpy2DAsync(A_d, pitch, A_h, width, width, numH, UPTKMemcpyHostToDevice,stream);
    EXPECT_EQ(ret, UPTKSuccess) << "UPTKMemcpy2D failed";

    ret = UPTKMemcpy2DAsync(B_h, width, A_d, pitch, width, numH, UPTKMemcpyDeviceToHost,stream);
    EXPECT_EQ(ret, UPTKSuccess) << "UPTKMemcpy2D failed";
