...
TEST(cudaMemory,cudaHostGetFlagsTest){
    float *h_A = NULL;
    float *d_A = NULL;
    unsigned int flagA, FlagA;
    FlagA = UPTKHostAllocMapped;
    int bufferSize = LEN*sizeof(float);
    UPTKError_t ret = UPTKSuccess;

    ret = UPTKHostAlloc((void**)&h_A, bufferSize, UPTKHostAllocMapped);
    EXPECT_EQ(ret, UPTKSuccess) << "UPTKHostAlloc failed";
