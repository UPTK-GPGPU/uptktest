...
TEST(cudaMemory,cudaMemcpyTest7){
    int numDevice = 0;
    double *buffer = NULL;
    double *hostBuffer = NULL;
    double *hostBufferB = NULL;
    int bufferSize = 1024*sizeof(double);
    UPTKError_t ret = UPTKSuccess;

    hostBuffer = (double *)malloc(bufferSize);
    ASSERT_TRUE(hostBuffer!=NULL) << "error: malloc failed";

    hostBufferB = (double *)malloc(bufferSize);
