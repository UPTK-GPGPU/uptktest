...
#define N 1024                       //Expand the N

TEST(cudaP2P,cudaMemcpyPeerAsyncTest3)
{
    ...
    if (numDevices <= 1)
    {
       ...
       ret = UPTKMemcpyPeerAsync(X_d,5,A_d,0,Nbytes,s);
       EXPECT_EQ(ret, UPTKSuccess);
       ret = UPTKMemcpyPeerAsync(Y_d,5,B_d,0,Nbytes,s);
       EXPECT_EQ(ret, UPTKSuccess);
