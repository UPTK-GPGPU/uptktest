...
__global__ void MatrixAddition_04(float* a, float *b, float*c) {
...
TEST(hipPerformanceEvent,hipEventRecord_Performance_16){
    ...
    cudaMemcpyAsync(Device_in, Host_in, NUM * sizeof(float), cudaMemcpyHostToDevice,stream1);

    // Record the start event
    cudaEventRecord(event, stream1);
    
    //Memory transfer from device to Host copy by using stream2
	cudaMemcpyAsync(Host_out, Device_out, NUM * sizeof(float), cudaMemcpyDeviceToHost,NULL);

    // wait for event in stream1
    cudaStreamWaitEvent(NULL,event,0);
    
    //hipLaunchKernelGGL(MatrixAddition_04, streamSize/WIDTH , WIDTH, 0, NULL , Device_in, Device_out, result);
   MatrixAddition_04<<<streamSize/WIDTH , WIDTH>>> (Device_in, Device_out, result);
