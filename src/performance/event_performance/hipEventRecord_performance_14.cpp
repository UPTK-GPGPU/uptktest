...
__global__ void MatrixAddition_02(float* a, float *b, float*c) {
...
TEST(hipPerformanceEvent,hipEventRecord_Performance_14){
    ...
    //Memory transfer from device to Host by using stream2
	cudaMemcpyAsync(Host_out, Device_out, NUM * sizeof(float), cudaMemcpyDeviceToHost,stream2);

    // wait for event in stream1
    cudaStreamWaitEvent(stream2,event,0);
    
    //hipLaunchKernelGGL(MatrixAddition_02, streamSize/WIDTH , WIDTH, 0, stream2 , Device_in, Device_out, result);
    MatrixAddition_02<<<streamSize/WIDTH , WIDTH, 0, stream2 >>>(Device_in, Device_out, result);
