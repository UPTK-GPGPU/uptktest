...
__global__ void matrixTranspose_6(float* out, float* in, const int width) {
...
void matrixTransposeCPUReference_6(float* output, float* input, const unsigned int width) {
...
TEST(hipPerformanceEvent,hipEventRecord_Performance_6) {
    float* Matrix;
    ...
    //hipLaunchKernelGGL(matrixTranspose_6, dim3(WIDTH / THREADS_PER_BLOCK_X, WIDTH / THREADS_PER_BLOCK_Y), dim3(THREADS_PER_BLOCK_X, THREADS_PER_BLOCK_Y), 0, 0, gpuTransposeMatrix, gpuMatrix, WIDTH);
    matrixTranspose_6<<<dim3(WIDTH / THREADS_PER_BLOCK_X, WIDTH / THREADS_PER_BLOCK_Y), dim3(THREADS_PER_BLOCK_X, THREADS_PER_BLOCK_Y)>>> (gpuTransposeMatrix, gpuMatrix, WIDTH);
    // Record the stop event
    cudaEventRecord(stop, NULL);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&eventMs, start, stop);
    ...
    matrixTransposeCPUReference_6(cpuTransposeMatrix, Matrix, WIDTH);
            // verify the results
