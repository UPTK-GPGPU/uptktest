...
TEST(hipPerformancepinD2H,hipPerformancepinD2H_2){
    int numDevice = 0;
    char *buffer = NULL;
    ...
    ret = cudaMemcpy(hostBuffer,tempbuffer[0], bufferSize, cudaMemcpyDeviceToHost);
    EXPECT_EQ(ret, cudaSuccess);
    //std::cout << "buffer=" << (void *)buffer << std::endl;
    for(int i=0; i< num;i++)
    {	    
        gettimeofday(&start,NULL);      
        ret = cudaMemcpy(hostBuffer,tempbuffer[i], bufferSize, cudaMemcpyDeviceToHost);
        EXPECT_EQ(ret, cudaSuccess);
