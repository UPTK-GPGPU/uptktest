...
TEST(cublas_level_3,cublas_trsm_example) {
    ...
    std::vector<data_type> B = {5.0, 7.0, 6.0, 8.0};
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasDtrsm(cublasH, side, uplo, transa, diag, m, n, &alpha, d_A, lda, d_B, ldb));
     ...
     */
    double eps = 1.0E-2;
    const std::vector<data_type> expect = {1.50,1.75,2.00,2.00};
    //printf("B (out)\n");
    check_matrix(m, n, B.data(), ldc,expect.data(),eps);
    //printf("=====\n");
