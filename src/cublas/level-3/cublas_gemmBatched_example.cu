...
TEST(cublas_level_3,cublas_gemmBatched_example) {
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasDgemmBatched(cublasH, transa, transb, m, n, k, &alpha, d_A_array, lda,
     ...
     */
    double eps = 1.0E-2;
    const std::vector<std::vector<data_type>> expect = {{19.0,43.0,22.0,50.0},{111.0,151.0,122.0,166.0}};
    //printf("C[0]\n");
    check_matrix(m, n, C_array[0].data(), ldc, expect[0].data(), eps);
    //printf("=====\n");

    //printf("C[1]\n");
    check_matrix(m, n, C_array[1].data(), ldc, expect[1].data(), eps);
    //printf("=====\n");
