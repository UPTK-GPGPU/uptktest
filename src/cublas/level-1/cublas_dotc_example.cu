...
TEST(cublas_level_1,cublas_dotc_example){
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasZdotc(cublasH, A.size(), d_A, incx, d_B, incy, &result));
     ...
     *   result = 178.44+-1.60j
     */
    //EXPECT_EQ(result.x, 178.44);
    //EXPECT_EQ(result.y, -1.60);
    double eps = 1.0E-2;
    EXPECT_EQ(std::abs(result.x- 178.44) > eps, false);
    EXPECT_EQ(std::abs(result.y-(-1.60)) > eps, false);

    // double eps = 1.0E-2;
    // std::vector<data_type> expect = {{178.44, -1.60}};
    // //printf("A (scaled)\n");
    // check_vector(result.size(), result,expect.size, eps);
    // printf("Result\n");
    // printf("%0.2f+%0.2fj\n", result.x, result.y);
    // printf("=====\n");
