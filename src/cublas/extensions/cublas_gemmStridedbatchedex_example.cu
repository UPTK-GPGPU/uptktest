...
TEST(cublas_extensions,cublas_gemmStridedbatchedex_example) {
    ...
    // printf("A[1]\n");
    // check_matrix(m, k, A.data() + (m * k), lda);
    // printf("=====\n");
    ...
    // printf("B[1]\n");
    // check_matrix(k, n, B.data() + (k * n), ldb);
    // printf("=====\n");
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasGemmStridedBatchedEx(
        ...
        traits<data_type>::cuda_data_type, ldc, strideC, batch_count, UPTK_R_64F,
        UPTKBLAS_GEMM_DEFAULT));
     ...
     */
    double eps = 1.0E-2;
    const std::vector<data_type> expect = {19.0,43.0,22.0,50.0,111.0,151.0,122.0,166.0};
    //printf("C[0]\n");
    check_matrix(m, n, C.data(), ldc, expect.data(), eps);
    //printf("=====\n");

    //printf("C[1]\n");
    check_matrix(m, n, C.data() + (m * n), ldc, expect.data()+ (m * n), eps);
    //printf("=====\n");
