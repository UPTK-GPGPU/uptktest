...
TEST(cublas_extensions,cublas_scalex_example) {
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasScalEx(cublasH, A.size(), &alpha, traits<data_type>::cuda_data_type, d_A,
                              traits<data_type>::cuda_data_type, incx,
     ...
     */
    double eps = 1.0E-2;
    const std::vector<data_type> expect = {2.2 ,4.4 ,6.6 ,8.8};
    //printf("A (scaled)\n");
    check_vector(A.size(), A.data(),expect.data(), eps);
    //printf("=====\n");
