...
TEST(cublas_extensions,cublas_nrm2ex_example) {
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasNrm2Ex(cublasH, A.size(), d_A, traits<data_type>::cuda_data_type, incx,
     ...
     *   Result = 5.48
     */
    //EXPECT_EQ(result, 5.48);
    double eps = 1.0E-2;
    EXPECT_EQ(std::abs(result - 5.48) > eps, false);

    // double eps = 1.0E-2;
    // const std::vector<data_type> expect = {5.48};
