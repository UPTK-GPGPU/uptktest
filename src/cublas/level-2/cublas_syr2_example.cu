...
TEST(cublas_level_2,cublas_syr2_example) {
    ...
    CUDA_CHECK(UPTKMemcpyAsync(d_A, A.data(), sizeof(data_type) * A.size(),
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasDsyr2(cublasH, uplo, n, &alpha, d_x, incx, d_y, incy, d_A, lda));

    /* step 4: copy data to host */
    CUDA_CHECK(UPTKMemcpyAsync(A.data(), d_A, sizeof(data_type) * A.size(),
     ...
     */
    double eps = 1.0E-2;
    const std::vector<data_type> expect = {71.0,3.00,85.0,100.0};
    //printf("A\n");
    check_matrix(m, n, A.data(), lda, expect.data(), eps);
    //printf("=====\n");
