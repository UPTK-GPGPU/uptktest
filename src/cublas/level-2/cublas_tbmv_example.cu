...
TEST(cublas_level_2,cublas_tbmv_example) {
    ...
    /* step 3: compute */
    CUBLAS_CHECK(UPTKblasDtbmv(cublasH, uplo, transa, diag, n, k, d_A, lda, d_x, incx));
     ...
     */
    double eps = 1.0E-2;
    const std::vector<data_type> expect = {27.00, 24.00};
    //printf("x\n");
    check_vector(x.size(), x.data(), expect.data(), eps);
    //printf("=====\n");
