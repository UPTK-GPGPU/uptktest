...
TEST(cufft, 1d_r2c_example)
{
    ...
    expect_data[1] = output_type(-8.f, 40.218716f);
    expect_data[2] = output_type(-8.f, 19.313709f);
    expect_data[3] = output_type(-8.f, 11.972846f);
    expect_data[4] = output_type(-8.f, 8.f);
    expect_data[5] = output_type(-8.f, 5.345428f);
    expect_data[6] = output_type(-8.f, 3.313708f);
    expect_data[7] = output_type(-8.f, 1.591299f);
    expect_data[8] = output_type(-8.f, 0.f);

    for (int i = 0; i < 8; i++)
    {
        EXPECT_NEAR(output[i].real(), expect_data[i].real(), 0.000001);
        EXPECT_NEAR(output[i].imag(), expect_data[i].imag(), 0.000001);
    }
