// ===--- d2zz2d_many_3d_inplace_basic.cu --------------------*- CUDA -*---===//
//
...
void d2zz2d_many_3d_inplace_basic()
{
  ...
  double *data_d;
  UPTKMalloc(&data_d, sizeof(double) * 64);
  UPTKMemcpy(data_d, forward_idata_h, sizeof(double) * 64, UPTKMemcpyHostToDevice);

  int n[3] = {4, 2, 3};
  size_t workSize;
  UPTKfftMakePlanMany(plan_fwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_D2Z, 2, &workSize);
  UPTKfftExecD2Z(plan_fwd, data_d, (double2 *)data_d);
  UPTKDeviceSynchronize();
  double2 forward_odata_h[32];
  UPTKMemcpy(forward_odata_h, data_d, sizeof(double) * 64, UPTKMemcpyDeviceToHost);
  ...
  UPTKfftMakePlanMany(plan_bwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_Z2D, 2, &workSize);
  UPTKfftExecZ2D(plan_bwd, (double2 *)data_d, data_d);
  UPTKDeviceSynchronize();
  double backward_odata_h[64];
  UPTKMemcpy(backward_odata_h, data_d, sizeof(double) * 64, UPTKMemcpyDeviceToHost);
...
TEST(cufft_runable, d2zz2d_many_3d_inplace_basic)
{
#define FUNC d2zz2d_many_3d_inplace_basic
  FUNC();
  UPTKDeviceSynchronize();
}
