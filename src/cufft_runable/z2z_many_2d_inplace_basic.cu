...
void z2z_many_2d_inplace_basic()
{
  ...
  double2 *data_d;
  UPTKMalloc(&data_d, sizeof(double2) * 12);
  UPTKMemcpy(data_d, forward_idata_h, sizeof(double2) * 12, UPTKMemcpyHostToDevice);

  int n[2] = {2, 3};
  size_t workSize;
  UPTKfftMakePlanMany(plan_fwd, 2, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_Z2Z, 2, &workSize);
  UPTKfftExecZ2Z(plan_fwd, data_d, data_d, UPTKFFT_FORWARD);
  UPTKDeviceSynchronize();
  double2 forward_odata_h[12];
  UPTKMemcpy(forward_odata_h, data_d, sizeof(double2) * 12, UPTKMemcpyDeviceToHost);
  ...
  UPTKfftMakePlanMany(plan_bwd, 2, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_Z2Z, 2, &workSize);
  UPTKfftExecZ2Z(plan_bwd, data_d, data_d, UPTKFFT_INVERSE);
  UPTKDeviceSynchronize();
  double2 backward_odata_h[12];
  UPTKMemcpy(backward_odata_h, data_d, sizeof(double2) * 12, UPTKMemcpyDeviceToHost);
...
TEST(cufft_runable, z2z_many_2d_inplace_basic)
{
