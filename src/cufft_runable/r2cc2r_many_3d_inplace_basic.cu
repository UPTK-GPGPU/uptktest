...
void r2cc2r_many_3d_inplace_basic()
{
  ...
  float *data_d;
  UPTKMalloc(&data_d, sizeof(float) * 64);
  UPTKMemcpy(data_d, forward_idata_h, sizeof(float) * 64, UPTKMemcpyHostToDevice);

  int n[3] = {4, 2, 3};
  size_t workSize;
  UPTKfftMakePlanMany(plan_fwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_R2C, 2, &workSize);
  UPTKfftExecR2C(plan_fwd, data_d, (float2 *)data_d);
  UPTKDeviceSynchronize();
  float2 forward_odata_h[32];
  UPTKMemcpy(forward_odata_h, data_d, sizeof(float) * 64, UPTKMemcpyDeviceToHost);
  ...
  UPTKfftMakePlanMany(plan_bwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_C2R, 2, &workSize);
  UPTKfftExecC2R(plan_bwd, (float2 *)data_d, data_d);
  UPTKDeviceSynchronize();
  float backward_odata_h[64];
  UPTKMemcpy(backward_odata_h, data_d, sizeof(float) * 64, UPTKMemcpyDeviceToHost);
...
TEST(cufft_runable, r2cc2r_many_3d_inplace_basic)
{
