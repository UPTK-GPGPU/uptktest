// ===--- c2c_many_3d_outofplace_basic.cu --------------------*- CUDA -*---===//
//
...
void c2c_many_3d_outofplace_basic()
{
  ...
  UPTKMalloc(&forward_idata_d, sizeof(float2) * 48);
  UPTKMalloc(&forward_odata_d, sizeof(float2) * 48);
  UPTKMalloc(&backward_odata_d, sizeof(float2) * 48);
  UPTKMemcpy(forward_idata_d, forward_idata_h, sizeof(float2) * 48, UPTKMemcpyHostToDevice);

  int n[3] = {2, 3, 4};
  UPTKfftPlanMany(&plan_fwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_C2C, 2);
  UPTKfftExecC2C(plan_fwd, forward_idata_d, forward_odata_d, UPTKFFT_FORWARD);
  UPTKDeviceSynchronize();
  float2 forward_odata_h[48];
  UPTKMemcpy(forward_odata_h, forward_odata_d, sizeof(float2) * 48, UPTKMemcpyDeviceToHost);
  ...
  UPTKfftPlanMany(&plan_bwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_C2C, 2);
  UPTKfftExecC2C(plan_bwd, forward_odata_d, backward_odata_d, UPTKFFT_INVERSE);
  UPTKDeviceSynchronize();
  float2 backward_odata_h[48];
  UPTKMemcpy(backward_odata_h, backward_odata_d, sizeof(float2) * 48, UPTKMemcpyDeviceToHost);
...
TEST(cufft_runable, c2c_many_3d_outofplace_basic)
{
#define FUNC c2c_many_3d_outofplace_basic
  FUNC();
  UPTKDeviceSynchronize();
}
