// ===--- z2z_many_3d_outofplace_basic.cu --------------------*- CUDA -*---===//
//
...
void z2z_many_3d_outofplace_basic()
{
  ...
  UPTKMalloc(&forward_idata_d, sizeof(double2) * 48);
  UPTKMalloc(&forward_odata_d, sizeof(double2) * 48);
  UPTKMalloc(&backward_odata_d, sizeof(double2) * 48);
  UPTKMemcpy(forward_idata_d, forward_idata_h, sizeof(double2) * 48, UPTKMemcpyHostToDevice);

  int n[3] = {2, 3, 4};
  UPTKfftPlanMany(&plan_fwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_Z2Z, 2);
  UPTKfftExecZ2Z(plan_fwd, forward_idata_d, forward_odata_d, UPTKFFT_FORWARD);
  UPTKDeviceSynchronize();
  double2 forward_odata_h[48];
  UPTKMemcpy(forward_odata_h, forward_odata_d, sizeof(double2) * 48, UPTKMemcpyDeviceToHost);
  ...
  compare(forward_odata_ref, forward_odata_h, 48);
  ...
  UPTKfftPlanMany(&plan_bwd, 3, n, nullptr, 0, 0, nullptr, 0, 0, UPTKFFT_Z2Z, 2);
  UPTKfftExecZ2Z(plan_bwd, forward_odata_d, backward_odata_d, UPTKFFT_INVERSE);
  UPTKDeviceSynchronize();
  double2 backward_odata_h[48];
  UPTKMemcpy(backward_odata_h, backward_odata_d, sizeof(double2) * 48, UPTKMemcpyDeviceToHost);
  ...
  compare(backward_odata_ref, backward_odata_h, 48);
...
TEST(cufft_runable, z2z_many_3d_outofplace_basic)
{
#define FUNC z2z_many_3d_outofplace_basic
  FUNC();
  UPTKDeviceSynchronize();
}
