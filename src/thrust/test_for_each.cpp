...
TYPED_TEST(ForEachTests, HostPathSimpleTest)
{
    ...
    for(auto size : sizes)
    {
        ...
        // Zero input memory
        if(size > 0)
            HIP_CHECK(cudaMemset(raw_ptr, 0, sizeof(T) * size));
        ...
        HIP_CHECK(cudaMemcpy(output.data(), raw_ptr, size * sizeof(T), cudaMemcpyDeviceToHost));
...
TYPED_TEST(ForEachTests, DevicePathSimpleTest)
{
    ...
    // Run for_each in [0; end] range
    size_t end = 375;
    simple_test_kernel<mark_processed_functor<T>><<<
                       dim3(1),
                       dim3(1),
                       0,
                       0>>> (
