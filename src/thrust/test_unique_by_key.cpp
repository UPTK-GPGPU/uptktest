...
void UniqueByKeyKernel(int const N, int *in_array, int *in_keys, int *out_size)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::pair<thrust::device_vector<int>::iterator,thrust::device_vector<int>::iterator>  d_result = thrust::unique_by_key(thrust::cuda::par,keys_begin, keys_end, in_begin);
