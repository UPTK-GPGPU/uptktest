...
void PartitionKernel(int const N, int* array)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::partition(thrust::cuda::par, begin, end,is_even<int>());
...
void PartitionCopyKernel(int const N, int* array, int* true_arr, int* false_arr, int* size_array)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::pair<iter,iter> end_pair = thrust::partition_copy(thrust::cuda::par, begin, end, true_begin, false_begin, is_even<int>());
...
//         thrust::device_ptr<int> begin(array);
//         thrust::device_ptr<int> end(array + N);
//         //TODO: The thrust::cuda::par throw exception, we should fix it
//         thrust::stable_partition(thrust::cuda::par, begin, end,is_even<int>());
//
//     }
// }
//
