...
bool supports_hmm()
{
    ...
    int device_id;
    HIP_CHECK_HMM(cudaGetDevice(&device_id));
    HIP_CHECK_HMM(cudaGetDeviceProperties(&device_prop, device_id));
...
TEST(DevicePtrTests, TestDevicePointerManipulationHmm)
{
    ...
    int* data;
    HIP_CHECK(cudaMallocManaged((void**)&data, 5 * sizeof(int)));
...
TYPED_TEST(DevicePtrPrimitiveTests, TestDevicePointerValue)
{
    ...
    for(auto size : sizes)
    {
        ...
        // Zero input memory
        if(size > 0)
            HIP_CHECK(cudaMemset(raw_ptr_begin, 0, sizeof(T) * size));
