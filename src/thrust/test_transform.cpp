...
void UnaryTransformKernel(int const N, int* input, int *output)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::transform(thrust::cuda::par, in_begin, in_end, out_begin, unary_transform<int>());
    }
}
...
void BinaryTransformKernel(int const N, int* input1, int* input2, int *output)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::device_ptr<int>   out_begin(output);
        thrust::transform(thrust::cuda::par, in1_begin, in1_end,in2_begin, out_begin, binary_transform<int>());
    }
}
