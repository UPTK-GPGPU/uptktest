...
void InclusiveScanByKeyKernel(int const N, int* in_array, int* keys_array, int *out_array)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::inclusive_scan_by_key(thrust::cuda::par, keys_begin, keys_end, in_begin, out_begin);
    }
}
...
void ExclusiveScanByKeyKernel(int const N, int* in_array, int* keys_array, int *out_array)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::exclusive_scan_by_key(thrust::cuda::par, keys_begin, keys_end, in_begin, out_begin);
    }
}
