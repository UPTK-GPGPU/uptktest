...
TEST(HipThrustCopy, DeviceToDevice)
{
    ...
    // Zero d_ptr1 memory
    HIP_CHECK(cudaMemset(thrust::raw_pointer_cast(d_ptr1), 0, sizeof(int) * size));
    HIP_CHECK(cudaMemset(thrust::raw_pointer_cast(d_ptr2), 0xdead, sizeof(int) * size));

    // Copy device->device
    thrust::copy(d_ptr1, d_ptr1 + 256, d_ptr2);

    std::vector<int> output(size);
    HIP_CHECK(cudaMemcpy(output.data(),
...
void CopyKernel(int const N, int* in_array, int *out_array)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::copy(thrust::cuda::par, in_begin, in_end, out_begin);
    }
}
...
void CopyIfKernel(int const N, int* in_array, int *out_array, int * out_size)
{
    if(threadIdx.x == 0)
    {
        ...
        thrust::device_vector<int>::iterator last = thrust::copy_if(thrust::cuda::par, in_begin, in_end, out_begin,is_even<int>());
