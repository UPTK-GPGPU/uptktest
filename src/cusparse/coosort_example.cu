...
TEST(cusparse, coosort_example) {
    ...
    for (int i = 0; i < nnz; i++) {
        if (h_rows[i]          != h_rows_ref[i]    ||
            ...
            EXPECT_EQ(h_rows[i] , h_rows_ref[i]);
            EXPECT_DOUBLE_EQ(h_columns[i] , h_columns_ref[i]);
            EXPECT_EQ(h_values_sorted[i] , h_values_ref[i]);
            EXPECT_EQ(h_permutation[i] , h_permutation_ref[i]);
            break;
        }
    }
