...
TEST(cuMemory, cuMemcpyDtoHAsyncTest) {
    cudaSetDevice(0);
    ...
    cudaMemcpyAsync(data, buffer, NUM * sizeof(float), cudaMemcpyHostToDevice, NULL);
    ...
    cuMemcpyDtoHAsync(gpuMatrix, (CUdeviceptr)data, NUM * sizeof(float), NULL);
