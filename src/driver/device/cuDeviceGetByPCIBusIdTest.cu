...
TEST(cuDevice,cuDeviceGetByPCIBusIdTest){
    ...
    for (int i = 0; i < deviceCount; i++) {
        ...
        int tempPciBusId = -1;
        int tempDeviceId = -1;
        ret1 = cuDeviceGetPCIBusId(&pciBusId[0], 13, i);//返回设备的一种总线标准总线标识字符串，重载以采取int设备标识。
        ...
        ret3 = cuDeviceGetByPCIBusId(&tempDeviceId, pciBusId);
        EXPECT_EQ(ret3, CUDA_SUCCESS);
