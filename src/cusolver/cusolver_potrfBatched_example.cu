...
TEST(cusolver, cusolver_potrfBatched_example) {
    ...
    double eps = 1.0E-2;
    std::vector<double> expect1={1.0,2.0,3.0,2.0,1.0,-1.0,3.0,5.0,1.41};
    check_matrix(m, m, L0.data(), lda, expect1  .data(), eps);
    //std::printf("=====\n");
    ...
    //     exit(1);
    // }
    EXPECT_EQ(infoArray[0],0);
    //std::printf("X0 = (matlab base-1)\n");
    //double eps = 1.0E-2;
    std::vector<double> expect2={10.5,-2.5,-1.5};
    check_matrix(m, 1, X0.data(), ldb, expect2.data(), eps);
    //std::printf("=====\n");
