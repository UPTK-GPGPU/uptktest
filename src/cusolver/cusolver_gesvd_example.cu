...
TEST(cusolver, cusolver_gesvd_example) {
    ...
    std::vector<double> expect1={7.07,1.04};
    check_matrix(n, 1, S.data(), n,expect1.data(),eps);
    //std::printf("=====\n");

    std::vector<double> expect2={-0.31,-0.91,-0.29,0.49,0.11,-0.87,0.82,-0.41,0.41};
    //std::printf("U = left singular vectors (matlab base-1)\n");
    check_matrix(m, m, U.data(), lda,expect2.data(),eps);
    //std::printf("=====\n");

    std::vector<double> expect3={-0.638,-0.769,0.0,-0.769,0.638,0.0};

    //std::printf("VT = right singular vectors (matlab base-1)\n");
    check_matrix(n, n, VT.data(), lda,expect3.data(),eps);
    //std::printf("=====\n");
