/*
 * Auto-generated smoke test for UPTKrandGetDirectionVectors64 (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKrandDirectionVectors64_t *dv64_holder{};
    UPTKrandStatus_t err;

    err = rand_setup_core(&generator, &dev_scratch, &stream_id, 1);
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGetDirectionVectors64 setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGetDirectionVectors64(&dv64_holder, (UPTKrandDirectionVectorSet_t)UPTKRAND_DIRECTION_VECTORS_64_JOEKUO6);

    printf("UPTKrandGetDirectionVectors64 -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGetDirectionVectors64 PASS\n");
    return 0;
}
