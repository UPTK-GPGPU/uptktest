/*
 * Auto-generated smoke test for UPTKrandSetPseudoRandomGeneratorSeed (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandSetPseudoRandomGeneratorSeed setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandSetPseudoRandomGeneratorSeed(generator, 0ull);

    printf("UPTKrandSetPseudoRandomGeneratorSeed -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandSetPseudoRandomGeneratorSeed PASS\n");
    return 0;
}
