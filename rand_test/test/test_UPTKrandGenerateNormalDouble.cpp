...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateNormalDouble setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateNormalDouble(generator, (double*)dev_scratch, (size_t)4, 1., 1.);

    printf("UPTKrandGenerateNormalDouble -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateNormalDouble PASS\n");
    return 0;
}
