/*
 * Auto-generated smoke test for UPTKrandGenerateLogNormal (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateLogNormal setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateLogNormal(generator, (float*)dev_scratch, (size_t)4, 0.f, 0.f);

    printf("UPTKrandGenerateLogNormal -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateLogNormal PASS\n");
    return 0;
}
