/*
 * Auto-generated smoke test for UPTKrandSetGeneratorOffset (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandSetGeneratorOffset setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandSetGeneratorOffset(generator, 0ull);

    printf("UPTKrandSetGeneratorOffset -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandSetGeneratorOffset PASS\n");
    return 0;
}
