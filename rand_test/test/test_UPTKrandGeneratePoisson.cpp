/*
 * Auto-generated smoke test for UPTKrandGeneratePoisson (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGeneratePoisson setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGeneratePoisson(generator, (unsigned int*)dev_scratch, (size_t)4, 1.);

    printf("UPTKrandGeneratePoisson -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGeneratePoisson PASS\n");
    return 0;
}
