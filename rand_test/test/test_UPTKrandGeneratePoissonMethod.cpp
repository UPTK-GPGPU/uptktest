/*
 * Auto-generated smoke test for UPTKrandGeneratePoissonMethod (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGeneratePoissonMethod setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGeneratePoissonMethod(generator, (unsigned int*)dev_scratch, (size_t)4, 1., (UPTKrandMethod_t)0);

    printf("UPTKrandGeneratePoissonMethod -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGeneratePoissonMethod PASS\n");
    return 0;
}
