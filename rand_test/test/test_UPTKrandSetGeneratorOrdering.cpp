/*
 * Auto-generated smoke test for UPTKrandSetGeneratorOrdering (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandSetGeneratorOrdering setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandSetGeneratorOrdering(generator, (UPTKrandOrdering_t)UPTKRAND_ORDERING_PSEUDO_DEFAULT);

    printf("UPTKrandSetGeneratorOrdering -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandSetGeneratorOrdering PASS\n");
    return 0;
}
