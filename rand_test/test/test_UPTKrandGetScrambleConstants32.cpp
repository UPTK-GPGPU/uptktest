/*
 * Auto-generated smoke test for UPTKrandGetScrambleConstants32 (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    unsigned int *scramble32{};
    UPTKrandStatus_t err;

    err = rand_setup_core(&generator, &dev_scratch, &stream_id, 1);
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGetScrambleConstants32 setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGetScrambleConstants32(&scramble32);

    printf("UPTKrandGetScrambleConstants32 -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGetScrambleConstants32 PASS\n");
    return 0;
}
