/*
 * Auto-generated smoke test for UPTKrandSetQuasiRandomGeneratorDimensions (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandSetQuasiRandomGeneratorDimensions setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandSetQuasiRandomGeneratorDimensions(generator, 0u);

    printf("UPTKrandSetQuasiRandomGeneratorDimensions -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandSetQuasiRandomGeneratorDimensions PASS\n");
    return 0;
}
