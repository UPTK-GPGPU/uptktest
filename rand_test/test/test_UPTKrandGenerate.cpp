/*
 * Auto-generated smoke test for UPTKrandGenerate (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerate setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerate(generator, (unsigned int*)dev_scratch, (size_t)4);

    printf("UPTKrandGenerate -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerate PASS\n");
    return 0;
}
