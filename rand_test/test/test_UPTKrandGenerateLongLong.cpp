/*
 * Auto-generated smoke test for UPTKrandGenerateLongLong (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateLongLong setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateLongLong(generator, (unsigned long long*)dev_scratch, (size_t)4);

    printf("UPTKrandGenerateLongLong -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateLongLong PASS\n");
    return 0;
}
