/*
 * Auto-generated smoke test for UPTKrandDestroyGenerator (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandDestroyGenerator setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandDestroyGenerator(generator);

    printf("UPTKrandDestroyGenerator -> %d\n", (int)err);



    rand_teardown_core((UPTKrandGenerator_t)(uintptr_t)0, dev_scratch, stream_id);

    printf("test_UPTKrandDestroyGenerator PASS\n");
    return 0;
}
