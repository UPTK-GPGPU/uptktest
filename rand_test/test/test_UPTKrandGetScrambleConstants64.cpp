/*
 * Auto-generated smoke test for UPTKrandGetScrambleConstants64 (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    unsigned long long *scramble64{};
    UPTKrandStatus_t err;

    err = rand_setup_core(&generator, &dev_scratch, &stream_id, 1);
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGetScrambleConstants64 setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGetScrambleConstants64(&scramble64);

    printf("UPTKrandGetScrambleConstants64 -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGetScrambleConstants64 PASS\n");
    return 0;
}
