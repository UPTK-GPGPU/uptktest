/*
 * Auto-generated smoke test for UPTKrandGenerateLogNormalDouble (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateLogNormalDouble setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateLogNormalDouble(generator, (double*)dev_scratch, (size_t)4, 1., 1.);

    printf("UPTKrandGenerateLogNormalDouble -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateLogNormalDouble PASS\n");
    return 0;
}
