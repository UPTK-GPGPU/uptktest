/*
 * Auto-generated smoke test for UPTKrandCreateGenerator (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandCreateGenerator bind failed\n");
        return 0;
    }
    if (cudaMalloc(&dev_scratch, 65536) != cudaSuccess || cudaStreamCreate(&stream_id) != cudaSuccess) {
        printf("test_skip: UPTKrandCreateGenerator cuda alloc/stream\n");
        return 0;
    }

    err = UPTKrandCreateGenerator(&generator, (UPTKrandRngType_t)UPTKRAND_RNG_PSEUDO_DEFAULT);
    printf("UPTKrandCreateGenerator -> %d\n", (int)err);
    if (generator)
        UPTKrandDestroyGenerator(generator);
    cudaFree(dev_scratch);
    cudaStreamDestroy(stream_id);
    printf("test_UPTKrandCreateGenerator PASS\n");
    return 0;
}
