/*
 * Auto-generated smoke test for UPTKrandGenerateUniform (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateUniform setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateUniform(generator, (float*)dev_scratch, (size_t)4);

    printf("UPTKrandGenerateUniform -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateUniform PASS\n");
    return 0;
}
