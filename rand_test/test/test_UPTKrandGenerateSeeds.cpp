/*
 * Auto-generated smoke test for UPTKrandGenerateSeeds (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandGenerateSeeds setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandGenerateSeeds(generator);

    printf("UPTKrandGenerateSeeds -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandGenerateSeeds PASS\n");
    return 0;
}
