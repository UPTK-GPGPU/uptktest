/*
 * Auto-generated smoke test for UPTKrandCreatePoissonDistribution (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    void *dev_scratch{};
    UPTKStream_t stream_id{};
    UPTKrandDiscreteDistribution_t discrete_distribution{};
    UPTKrandStatus_t err;

    err = rand_setup_core(&generator, &dev_scratch, &stream_id, 1);
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandCreatePoissonDistribution setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandCreatePoissonDistribution(1., &discrete_distribution);

    printf("UPTKrandCreatePoissonDistribution -> %d\n", (int)err);
    ...
    printf("test_UPTKrandCreatePoissonDistribution PASS\n");
    return 0;
}
