/*
 * Auto-generated smoke test for UPTKrandSetStream (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandSetStream setup failed (%d)\n", (int)err);
        return 0;
    }
    err = UPTKrandSetStream(generator, stream_id);

    printf("UPTKrandSetStream -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandSetStream PASS\n");
    return 0;
}
