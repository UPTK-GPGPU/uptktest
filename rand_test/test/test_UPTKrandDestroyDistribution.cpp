/*
 * Auto-generated smoke test for UPTKrandDestroyDistribution (rand_fun_convert.cpp).
...
int main(void)
{
    ...
    if (err != UPTKRAND_STATUS_SUCCESS) {
        printf("test_skip: UPTKrandDestroyDistribution setup failed (%d)\n", (int)err);
        return 0;
    }
    ...
    err = UPTKrandDestroyDistribution(discrete_distribution);

    printf("UPTKrandDestroyDistribution -> %d\n", (int)err);



    rand_teardown_core(generator, dev_scratch, stream_id);

    printf("test_UPTKrandDestroyDistribution PASS\n");
    return 0;
}
